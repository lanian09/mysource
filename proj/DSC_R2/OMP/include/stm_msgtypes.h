...
typedef struct {
	unsigned int    cpu_cnt;
	unsigned int    avg_cpu[MAX_SCE_CPU_CNT];
	unsigned int    max_cpu[MAX_SCE_CPU_CNT];
	unsigned int    mem_cnt;
	unsigned int    avg_mem[MAX_SCE_MEM_CNT];
	unsigned int    max_mem[MAX_SCE_MEM_CNT];
	unsigned int    avg_disk;
	unsigned int    max_disk;
	/* hjjung_20100823 */
	unsigned int    avg_user;
	unsigned int    max_user;
} STM_LoadSceStatMsgType;

typedef struct {
	STM_LoadMPStatMsgType ompInfo;
	STM_LoadCommStatMsgType pdInfo[MAX_SCE_DEV_NUM];
	STM_LoadSceStatMsgType	sceInfo[MAX_SCE_DEV_NUM];
...
typedef struct { // 모든 시스템에서 공통적으로 발생하는 장애 정보
    ...
    unsigned int    optlan[SFM_ALM_CRITICAL]; // Mirroring port 장애   // by helca 09.13
    unsigned int    mp_hw[SFM_ALM_CRITICAL]; // MP_H/W 장애(disk, power, fan, eth)  // by helca 09.13
    unsigned int    net_nms[SFM_ALM_CRITICAL];
    unsigned int    sess_ntp[2][SFM_ALM_CRITICAL];
	...
	unsigned int    pdpower[2][SFM_ALM_CRITICAL]; // 20110424 by dcham
	/*    unsigned int    queue[SFM_ALM_CRITICAL];	*/// Queue 
	unsigned int    sceCpu[2][SFM_ALM_CRITICAL];
	unsigned int    sceMem[2][SFM_ALM_CRITICAL];
    unsigned int    sceDisk[2][SFM_ALM_CRITICAL];
    unsigned int    scePwr[2][SFM_ALM_CRITICAL];
    unsigned int    sceFan[2][SFM_ALM_CRITICAL];
    unsigned int    sceTemp[2][SFM_ALM_CRITICAL];
    unsigned int    sceVolt[2][SFM_ALM_CRITICAL];
    unsigned int    scePort[2][SFM_ALM_CRITICAL];
    unsigned int    sceRdr[2][SFM_ALM_CRITICAL];
    unsigned int    sceRdrConn[2][SFM_ALM_CRITICAL];
    unsigned int    sceStatus[2][SFM_ALM_CRITICAL];
    unsigned int    l2Cpu[2][SFM_ALM_CRITICAL];
    unsigned int    l2Mem[2][SFM_ALM_CRITICAL];
    unsigned int    l2Port[2][SFM_ALM_CRITICAL];
	/* hjjung_20100823 */
    unsigned int    sceUser[2][SFM_ALM_CRITICAL];
	/* hjjung_20100823 */
    unsigned int    legSession[SFM_ALM_CRITICAL];

	/* added by uamyd 20110209. LOGON 성공율 감시를 위한 */
    unsigned int    logonSuccRate[SFM_ALM_CRITICAL];
	/* added by uamyd 20110424. LOGOUT 성공율 감시를 위한 */
    unsigned int    logoutSuccRate[SFM_ALM_CRITICAL];
    unsigned int    smCh[SFM_MAX_SM_CH_CNT][SFM_ALM_CRITICAL];
    unsigned int    tpsStatus[SFM_ALM_CRITICAL]; /* TPS 알람 count, added by dcham 2011.05.25 */
} STM_CommFltStat;
...
typedef struct {
	...
	unsigned int	succ_anaaa[SFM_ALM_CRITICAL];	
	unsigned int    succ_vt[SFM_ALM_CRITICAL];	
	unsigned int    succ_radius[SFM_ALM_CRITICAL];	
	...
	unsigned int    scm_fault[SFM_ALM_CRITICAL]; // 20100915 by dcham
	//unsigned int    dualAct[SFM_ALM_CRITICAL];
	//unsigned int    dualStd[SFM_ALM_CRITICAL];
	//unsigned int    dualTimeOut[SFM_ALM_CRITICAL];	
} STM_SpecFltStat_BSD;

...
typedef struct {
	...
	unsigned int		bsdmTimeoutAlarm;   /* 1 : Normal, 2 : AbNormal(alarm) */
...
typedef struct{
	...
	unsigned char   StsInfo[COMM_MAX_NAME_LEN];     /**< inteface °æ¿i ip address **/

#define SFM_HW_STATUS_ALIVE      0
#define SFM_SW_STATUS_DEAD      1
...
typedef struct {
	...
	SFM_SysSts				sysSts;
	SFM_HWSts               sysHW[SFM_MAX_HPUX_HW_COM];     // H/W,S/W 정보 40
	SFM_Duia				duia;
	SFM_SM_CONN				smConn[MAX_RLEG_CNT+1];			// SM Connection Status Connect:1, Disconnect:-1
	unsigned char			cps_over_alm_flag;              // CPS 알람 체크(<== RLEG)

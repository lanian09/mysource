...
int print_aaa_msg(st_AAAREQ *aaa_msg, FILE *OutFd, st_SrchInfo_t  *pSrchInfo);
...
int OctetPrint2( UCHAR *szBuf, UCHAR *szSubnet )
{
    int   i;
...
int print_aaa_msg(st_AAAREQ *aaa_msg, FILE *OutFd, st_SrchInfo_t  *pSrchInfo)
{
	...
	char        szCorrllID[8+1];
    UCHAR       szBSID[32], szSubnet[MAX_SUBNET_SIZE+3];
	...
	{
		sprintf(buffer,"%s",str_time(aaa_msg->stInfo.uiTimeStamp));
	}
    */
	len = strlen(buffer);
    ...
    */
    ...
    for (i=0; (i==0) || (i<aaa_msg->dUDRCount); i++)
    {
        ...
        else if(pSrchInfo->iViewType == VIEWTYPE_LINE) 
        {
            sprintf(&buffer[len],"< ");
            len = strlen(buffer);
        }
        ...
        if (aaa_msg->stInfo.ucCallStatIDF > 0)
        {
            if (pSrchInfo->iViewType == VIEWTYPE_NORM)
            {
                if(i==0)
                {
                    sprintf(&buffer[len],"CALLING-STATION-ID                 = [%s]\n",aaa_msg->stInfo.szMIN);
                }
            }
            else
        ...
        if (aaa_msg->stInfo.ucFramedIPF > 0)
        {
            ...
            if (pSrchInfo->iViewType == VIEWTYPE_NORM)
            {
                if(i==0)
                {
                    sprintf(&buffer[len],"FRAMED-IP-ADDRESS                  = [%s]\n",inet_ntoa(inaddr));
                }
            }
            else
        ...
        if (aaa_msg->stInfo.ucUserNameF > 0)
        {
            if (pSrchInfo->iViewType == VIEWTYPE_NORM)
            {
                if(i==0)
                {
                    sprintf(&buffer[len],"USER-NAME                          = [%s]\n",aaa_msg->stInfo.szUserName);
                }
            }
            else
        ...
        if (aaa_msg->stInfo.ucSessContinueF > 0)
        {
            if (pSrchInfo->iViewType == VIEWTYPE_NORM)
            {
                if(i==0)
                {
                    sprintf(&buffer[len],"SESSION-CONT                       = [%d]\n",aaa_msg->stInfo.uiSessContinue);
                }
            }
            else
        ...
        if (aaa_msg->stInfo.ucHAIPF > 0)
        {
            ...
            if (pSrchInfo->iViewType == VIEWTYPE_NORM)
            {
                if(i==0)
                {
                    sprintf(&buffer[len],"HA-IP-ADDR                         = [%s]\n",inet_ntoa(inaddr));
                }
            }
            else
                sprintf(&buffer[len],"%s< ",inet_ntoa(inaddr));
        ...
        if (aaa_msg->stInfo.ucNASIPF > 0)
        {
            ...
            if (pSrchInfo->iViewType == VIEWTYPE_NORM )
            {
                if(i==0)
                {
                    sprintf(&buffer[len],"NAS-IP-ADDRESS                     = [%s]\n",inet_ntoa(inaddr));
                }
            }
            else
                sprintf(&buffer[len],"%s< ",inet_ntoa(inaddr));
        ...
        if (aaa_msg->stInfo.ucPCFIPF > 0)
        {
            ...
            if (pSrchInfo->iViewType == VIEWTYPE_NORM)
            {
                if(i==0)
                {
                    sprintf(&buffer[len],"PCF-IP-ADDR                        = [%s]\n",inet_ntoa(inaddr));
                }
            }
            else
                sprintf(&buffer[len],"%s< ",inet_ntoa(inaddr));
        ...
        if (aaa_msg->stInfo.ucFwdFCHMuxF > 0)
        {
            if (pSrchInfo->iViewType == VIEWTYPE_NORM)
            {
                if(i==0)
                {
                    sprintf(&buffer[len],"F-FCH-MUX                          = [%d]\n",aaa_msg->stInfo.dFwdFCHMux);
                }
            }
            else
        ...
        if (aaa_msg->stInfo.ucRevFCHMuxF > 0)
        {
            if (pSrchInfo->iViewType == VIEWTYPE_NORM) 
            {
                if(i==0)
                {
                    sprintf(&buffer[len],"R-FCH-MUX                          = [%d]\n",aaa_msg->stInfo.dFwdFCHMux);
                }
            }
            else
        ...
        if (aaa_msg->stInfo.ucAcctOutOctF > 0)
        {
            if (pSrchInfo->iViewType == VIEWTYPE_NORM)
            {
                if(i==0)
                {
                    sprintf(&buffer[len],"ACCT-OUTPUT-OCTETS                 = [%d]\n",aaa_msg->stInfo.dAcctOutOct);
                }
            }
            else
        ...
        if (aaa_msg->stInfo.ucAcctInOctF > 0)
        {
            if (pSrchInfo->iViewType == VIEWTYPE_NORM)
            {
                if(i==0)
                {
                    sprintf(&buffer[len],"ACCT-INPUT-OCTETS                  = [%d]\n",aaa_msg->stInfo.dAcctInOct);
                }
            }
            else
        ...
        if (aaa_msg->stInfo.ucBadPPPFrameCntF > 0)
        {
            if (pSrchInfo->iViewType == VIEWTYPE_NORM)
            {
                if(i==0)
                {
                    sprintf(&buffer[len],"BAD-FRAME-COUNT                    = [%d]\n",aaa_msg->stInfo.dBadPPPFrameCnt);
                }
            }
            else
        ...
        if (aaa_msg->stInfo.ucEventTimeF > 0)
        {
            if (pSrchInfo->iViewType == VIEWTYPE_NORM)
            {
                if(i==0)
                { 
                    sprintf(&buffer[len],"EVENT-TIMESTAMP                    = [%s]\n",
        ...
        if (aaa_msg->stInfo.ucNumActF > 0)
        {
            if (pSrchInfo->iViewType == VIEWTYPE_NORM)
            {
                if(i==0)
                {
                    sprintf(&buffer[len],"NUM-ACTIVE                         = [%d]\n",aaa_msg->stInfo.dNumAct);
                }
            }
            else
        ...
        if (aaa_msg->stInfo.ucTermSDBOctCntF > 0)
        {
            if (pSrchInfo->iViewType == VIEWTYPE_NORM)
            {
                if(i==0)
                {
                    sprintf(&buffer[len],"SDB-INPUT-OCTETS                   = [%d]\n",aaa_msg->stInfo.dTermSDBOctCnt);
                }
            }
            else
        ...
        if (aaa_msg->stInfo.ucOrgNumSDBF > 0)
        {
            if (pSrchInfo->iViewType == VIEWTYPE_NORM)
            {
                if(i==0)
                {
                    sprintf(&buffer[len],"SDB-OUTPUT-OCTETS                  = [%d]\n",aaa_msg->stInfo.dTermSDBOctCnt);
                }
            }
            else
        ...
        if (aaa_msg->stInfo.ucTermNumSDBF > 0)
        {
            if (pSrchInfo->iViewType == VIEWTYPE_NORM)
            {
                if(i==0)
                {
                    sprintf(&buffer[len],"NUMSDB-INPUT                       = [%d]\n",aaa_msg->stInfo.dTermNumSDB);
                }
            }
            else
        ...
        if (aaa_msg->stInfo.ucOrgNumSDBF > 0)
        {
            if (pSrchInfo->iViewType == VIEWTYPE_NORM)
            {
                if(i==0)
                {
                    sprintf(&buffer[len],"NUMSDB-OUTPUT                      = [%d]\n",aaa_msg->stInfo.dTermNumSDB);
                }
            }
            else
        ...
        if (aaa_msg->stInfo.ucRcvHDLCOctF > 0)
        {
            if (pSrchInfo->iViewType == VIEWTYPE_NORM)
            {
                if(i==0)
                {
                    sprintf(&buffer[len],"NUM-BYTES-RECIVED-TOTAL            = [%d]\n",aaa_msg->stInfo.dRcvHDLCOct);
                }
            }
            else
        ...
        if (aaa_msg->stInfo.ucInMIPSigCntF > 0)
        {
            if (pSrchInfo->iViewType == VIEWTYPE_NORM)
            {
                if(i==0)
                {
                    sprintf(&buffer[len],"MIP-SIGNALING-INBOUND-COUNT        = [%d]\n",aaa_msg->stInfo.dInMIPSigCnt);
                }
            }
            else
        ...
        if (aaa_msg->stInfo.ucOutMIPSigCntF > 0)
        {
            if (pSrchInfo->iViewType == VIEWTYPE_NORM)
            {
                if(i==0)
                {
                    sprintf(&buffer[len],"MIP-SIGNALING-OUTBOUND-COUNT       = [%d]\n",aaa_msg->stInfo.dInMIPSigCnt);
                }
            }
            else
        ...
        if (aaa_msg->stInfo.ucAirQoSF > 0)
        {
            if (pSrchInfo->iViewType == VIEWTYPE_NORM)
            {
                if(i==0)
                {
                    sprintf(&buffer[len],"AIR-PRIORITY                       = [%d]\n",aaa_msg->stInfo.dAirQoS);
                }
            }
            else
        ...
        {
            if ((pSrchInfo->iViewType == VIEWTYPE_NORM) && (i == 0))
                    sprintf(&buffer[len],"H-BIT                              = [%d]\n",aaa_msg->stInfo.uiHBIT);
            else
	...
	// 071203, poopee
        /* Subnet */
        if (aaa_msg->stInfo.ucSubnetF > 0)
        {
            OctetPrint2( (UCHAR *)aaa_msg->stInfo.szSubnet, szSubnet );

            if (pSrchInfo->iViewType == VIEWTYPE_NORM)
            {
                if(i==0)
                {
                    sprintf(&buffer[len],"SUBNET                             = [%s]\n", szSubnet);
                }
            }
            else
                sprintf(&buffer[len],"%s< ", szSubnet);
        ...
        */
        ...
        {
            if (pSrchInfo->iViewType == VIEWTYPE_NORM )
                sprintf(&buffer[len],"DOWNLOAD_TYPE                      = [%d]\n",aaa_msg->stUDRInfo[i].dCType);
            else
		...
		*/
        ...
        if( aaa_msg->stUDRInfo[i].ucContentCodeF > 0x00 )
        {
            if (pSrchInfo->iViewType == VIEWTYPE_NORM )
                ...
                sprintf(&buffer[len],"CONTENT-ID                         = [%s]\n",aaa_msg->stUDRInfo[i].szContentCode);
            else
	...
	if( aaa_msg->stUDRInfo[i].ucCallerMinF > 0x00 )
        ...
        if( aaa_msg->stUDRInfo[i].ucTranTermReasonF > 0x00 )
        {
            if (pSrchInfo->iViewType == VIEWTYPE_NORM )
                ...
                sprintf(&buffer[len],"UDR-GENERATION-REASON     = [%d]\n",

...
int ProcessOriProcess()
{
    ...
    if( gSrchInfo.iSrchType == SRCHTYPE_FILE )
    {
		for(i=0; i<gSrchInfo.iFileCnt; i++)
		{
			...
			tmpChar++;
			if( strlen(tmpChar) == TXN_GZFILE_LEN )
			{
...
int get_para(int cnt, char *para[])
{
    ...
    if ( strlen(gSrchInfo.szOutFile) == 0)
    {
        if (gSrchInfo.iSrchType != SRCHTYPE_FILE)
        {
            ...
            {
                sprintf(gSrchInfo.szOutFile, "%s/SRCHMSG_%s_%s_%s_LOGID%d.RSLT",
...
GetInputFileName()
{
    ...
    while(1)
    {
        ...
        fprintf(stdout, " 입력이 완료되었으면 \"quit\"을 혹은 Enter를 기입함 \n");
...
GetInputTime()
{
    ...
    while(1)
    {
        ...
        fprintf(stdout, "Enter Search Start Time (Format : YYMMDDhhmm ex) 0609071330) : ");
        ...
        if( time_buf[0] == '\n')
        {
            fprintf(stdout, "Please Enter Illegal Start Time Format !!!\n");
            continue;
        }
        else
        {
    ...
    {
        memset(time_buf, 0x00, MAX_FILENAME_LEN);
        fprintf(stdout, "Enter Search End Time (Format : YYMMDDhhmm ex) 0609071330) : ");
        while (fgets(time_buf, sizeof(time_buf), stdin) == NULL)
        {
        }
        ...
        if( time_buf[0] == '\n')
        {
            fprintf(stdout, "Please Enter Illegal End Time Format !!!\n");
            continue;
        }
        else
        {
...
LogIdView()
{
    ...
    fprintf(stdout, "1. LOGID VIEW Search Time \n");
    fprintf(stdout, "9. Goto Main Menu \n");
    fprintf(stdout, "0. EXIT \n");
    fprintf(stdout, "=============================================\n");
    fprintf(stdout, "Input : ");

                     ...
                     FILE *OutFd, st_SrchInfo_t  *pSrchInfo)
{
	...
	if (pSrchInfo->iViewType == VIEWTYPE_NORM)
	{
		sprintf(buffer,"-------------------------------------------------------\n");
		len = strlen(buffer);
		sprintf(&buffer[len],"[%s]\n",str_time(CVT_INT_CP(pDbFetch->iLogTime)));
	}
	len = strlen(buffer);
    ...
    if (pSrchInfo->iViewType == VIEWTYPE_NORM)
        sprintf(&buffer[len],"LOG-ID                             = [%d]\n", 
                CVT_INT_CP(pDbFetch->iLogId) );
    else
        sprintf(&buffer[len],"%d; ", CVT_INT_CP(pDbFetch->iLogId));
    len = strlen(buffer);
    ...
    if (pSrchInfo->iViewType == VIEWTYPE_NORM)
        ...
        sprintf(&buffer[len],"%s; ", str_time(CVT_INT_CP(pDbFetch->iLogTime)));
    len = strlen(buffer);
    
    /* FLAG */
    if (pSrchInfo->iViewType == VIEWTYPE_NORM)
        sprintf(&buffer[len],"FLAG                               = [%d]\n", 
                CVT_INT_CP(pDbFetch->iFlag));
    else
        sprintf(&buffer[len],"%d; ", CVT_INT_CP(pDbFetch->iFlag));
    len = strlen(buffer);

...
AppFdTbl	nms_fd_tbl[MAX_NMS_CON];
...
analysis_event (int *s_fd)
{
	...
	for (i=0; i<MAX_NMS_CON; i++) {
		if (nms_fd_tbl[i].fd > 0) {
			if (FD_ISSET (nms_fd_tbl[i].fd, &rfd_set)) {
				*s_fd = nms_fd_tbl[i].fd;
...
del_fd_tbl (int sfd)
{
	...
	for (i=0; i<MAX_NMS_CON; i++) {
		if (nms_fd_tbl[i].fd == sfd) {
			memset (&nms_fd_tbl[i], 0, sizeof (AppFdTbl));
//lala

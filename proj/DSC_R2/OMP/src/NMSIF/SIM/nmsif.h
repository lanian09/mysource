...
#define	MAX_NMS_CON			100

#define	FD_TYPE_LISTEN		100
#define	FD_TYPE_DATA		101

typedef struct{
	int		fd;			// listen fd or accept fd
	int		port;		// listen port
	int		ipaddr;		// local ipaddr or nms ipaddr
	int		ptype;		// fd type (FD_TYPE_LISTEN/~DATA)
	int		rxTime;		// recent time to receive pkt
} AppFdTbl;
...
typedef struct {
	int			msgId;				// same with IxpcQMsgHeadType 
	char		segFlag;
	char		dummy[2];
	short		byteOrderFlag;
	short		bodyLen;
...
typedef struct {
	int			identifier;			// kind of statistics
	int			period;				// 5min/1hour/1day
	int			order;				// 1~xx, xx = number of statistics
} NmsStatInfo;
...
*/
/* 3.5.1 console data => one way primitive (BSDM->NMS) 
*/
//  NMS                          BSD
//   |        CONSOLE_DATA        |
//   |<---------------------------|
...
/* 3.5.2 alarm/fault => two way primitive (BSDM<->NMS) 
*/
//  NMS                          BSD
//   |      INIT_HWALM_REQ        |
//   |--------------------------->|
...
#define	PRMT_INIT_SWALM_REQ			(MT_ALARM_FAULT | MT_INITIAL | MT_SW_INFO | MT_DATA_RCV_OK)
...
/* 3.5.3 mmc => two way primitive (BSDM<->NMS) 
*/
//  NMS                          BSD
//   |        MMC_REQUEST         |
//   |--------------------------->|
...
/* 3.5.4 config => two way primitive (BSDM<->NMS) 
*/
//  NMS                          BSD
//   |        HW_EQUIP_REQ        |
//   |--------------------------->|
...
*/
#define	OIDCONF_FILE				"oidconfig"

#define	NMS_STAT_NUM				9	/* fault, hw_fault, load, mean, radius,
										 * svc_tr, svc_ttr, trans, uawap
										 */ 
...
typedef struct {
	...
	int		statid_base;
	int		oid_base;
	char	db_name[NMS_STAT_NUM][NUM_OF_PERIOD][40];
	int		item_base[NMS_STAT_NUM];
	int		key_oid[NMS_STAT_NUM];
	int		s_col[NMS_STAT_NUM];
	int		e_col[NMS_STAT_NUM];
	int		rcv_flag[NMS_STAT_NUM][NUM_OF_PERIOD];		/* RESET_RCV_FLAG or SET_RCV_FLAG */
} OidInfo;
#endif

typedef struct {
	char	db_name[NUM_OF_PERIOD][40];
	int		item_base;
	int		key_oid;
	int		s_col;
	int		e_col;
	int		rcv_flag[NUM_OF_PERIOD];		/* RESET_RCV_FLAG or SET_RCV_FLAG */
} OidInfo;

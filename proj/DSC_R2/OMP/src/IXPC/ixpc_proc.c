	...
	*/
	for (i=0; i<SYSCONF_MAX_ASSO_SYS_NUM; i++)
	{
		if (!sockRoutTbl[i].pres)
			continue;
		if (!strcasecmp(remoteAddr, sockRoutTbl[i].ipAddrPri) ||
			!strcasecmp(remoteAddr, sockRoutTbl[i].ipAddrSec) ||
			//!strcasecmp(remoteAddr, "192.168.100.15")		// yhshin , 15에 넣고, 
			!strcasecmp(remoteAddr, VRTS_IP_ADDR)		// yhshin , 15에 넣고, 
			) {
...
#else
					if ( !strcasecmp ( sockRoutTbl[i].sysName,ixpcCON->ixpcCon[i].name ) ){
						//fprintf (stderr," %s !!!!disconnect\n", sockRoutTbl[i].sysName );
...
// 07.17 jjinri	IxpcQMsgType	rxIxpcQMsg1;
...
//printf("sockBodyLen: %d\n", sockBodyLen);	
//jean printf("**1.ixpc_recvEventRxPort=msgid=%d:bodylen=%d\n",rxIxpcQMsg->head.msgId, rxIxpcQMsg->head.bodyLen);
...
//printf("**2.ixpc_recvEventRxPort=msgid=%d:bodylen=%d,%s:%s\n",rxIxpcQMsg->head.msgId, rxIxpcQMsg->head.bodyLen, rxIxpcQMsg->head.dstSysName, rxIxpcQMsg->head.dstAppName);

// by helca
	rxSysCommMsg = (SFM_SysCommMsgType*)rxIxpcQMsg->body;
//printf("qID-1: %d\n\n", ntohl(rxSysCommMsg->loc_que_sts[1].qID));
//printf("qID-1: %d qKEY_1: %d qNAME: %s qNUM: %d cBYTES:%d qBYTES:%d \n\n", rxSysCommMsg->loc_que_sts[1].qID, 
//	rxSysCommMsg->loc_que_sts[1].qKEY, rxSysCommMsg->loc_que_sts[1].qNAME,
//	rxSysCommMsg->loc_que_sts[1].qNUM, rxSysCommMsg->loc_que_sts[1].cBYTES, rxSysCommMsg->loc_que_sts[1].qBYTES);
...
{
// 0717 jjinri	int		ret,fd,i;
	int		fd,i;

	if ((fd = socklib_connect (sockRoutTbl[index].ipAddrPri, ixpcPortNum)) > 0) {
		sockRoutTbl[index].sockTxFd = fd;
		sockRoutTbl[index].lastTxTime = currentTime;
		for (i=0; i<SYSCONF_MAX_GROUP_MEMBER; i++){
			if ( ixpcCON->ixpcCon[i].name[0] == NULL ) continue;
	...
	if ((fd = socklib_connect (sockRoutTbl[index].ipAddrSec, ixpcPortNum)) > 0) {
		sockRoutTbl[index].sockTxFd = fd;
		sockRoutTbl[index].lastTxTime = currentTime;
		for (i=0; i<SYSCONF_MAX_GROUP_MEMBER; i++){
			if ( ixpcCON->ixpcCon[i].name[0] == NULL ) continue;

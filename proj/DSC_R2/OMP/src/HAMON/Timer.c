...
{                                                                                
    ...
    for(pp=&hash[0];*pp;pp=&((*pp)->next)) {                                   
		...
		t1 = (*pp)->t.time * 1000 + (*pp)->t.millitm;
		t2 = p->t.time * 1000 + p->t.millitm;
        if(t1 >= t2) break;
	}
    if((p->next = *pp))                                                          
        p->next->prev = &p->next;                                                
...
timer_check(void* args) 
{
	...
	while (1) {
    	...
    	for(p=hash[0];p;p=p->next) {                                   
			...
			t1 = p->t.time * 1000 + p->t.millitm;
			t2 = now.time * 1000 + now.millitm;
        	if(t1 > t2) break;
...
void
cbSetTimer(void (*func)(int, void*), int key, void* data, unsigned int msec)
{
    if (!callbacked) {

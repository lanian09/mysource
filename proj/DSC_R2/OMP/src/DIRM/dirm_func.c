...
#define TAP_POWER_MAX		2		/* POWER 갯수 20110424 by dcham */ 
...
RETRY_TAP_CHECK:
	...
	{
	    if(i==TAP_LINK_MAX) // manage port는 다른 곳에서... 20110424 by dcham
			continue;
	...
	/*
	   ...
	   /DSC/SCRIPT/snmp-tap.sh tapa | tapb
		...
		....
	 */
	// TAP_POWER_MAX까지 확장 20110424 by dcham
	i=0; // init
    bzero(buf, _SIZE);
...
GO_OUT:
	...
	for(i=0; i<(TAP_LINK_MAX+TAP_POWER_MAX); i++)
	{
		if(i<TAP_LINK_MAX){                         // i(inline/monitor port) : 0~21
			...
			pDirectPwr[pwrIdx].level= 4; // pwrIdx : 0~1 NOT_EQUIP 처리 added by dcham 2011.05.11

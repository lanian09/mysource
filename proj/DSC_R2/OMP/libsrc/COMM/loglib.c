...
{
	int		ret;
	char   	buff[4096*2], optBuf[256], tmp[80];
	va_list	args;
	struct timeval	curr;
...
openNewHourlyFile:
	...
	*/
	yyyy = (char*)strtok_r(sufNew, "/", &next);
	mm = (char*)strtok_r(next,"/",&ddhh);
...
openNewHourlyFile:
	...
	*/
	yyyy = (char*)strtok_r(sufNew, "/", &next);
	mm = (char*)strtok_r(next,"/",&last);
	dd = (char*)strtok_r(last,"/",&hh);

...
int socklib_sndMsg (int fd, char *buff, int buffLen)
{
	...
	*/
	// yhshin
	/*
	sockMsg = (SockLibMsgType *)buff;
    sockMsg->head.mapType = htonl(sockMsg->head.mapType);
    sockMsg->head.bodyLen = htonl(sockMsg->head.bodyLen);

	*/
	head = (SockLibHeadType*)buff;
#if 0 /* by june */
	fprintf(stderr, "socklib_sndMsg head->mapType: %d, %d \n", ntohl(head->mapType), ntohl(head->bodyLen));
#endif
...
*	- select로 확인하는데 select fail 시 fd를 강제로 close한다.
------------------------------------------------------------------------------*/
int socklib_sndMsg_hdr_chg (int fd, char *buff, int buffLen)
{
	int		i,len,wLen,ret;
	struct timeval	pollTmr;
	fd_set	wFdSet;
	...
	*/
	...
	head = (SockLibHeadType *)buff;
    head->mapType = htonl(head->mapType);
    head->bodyLen = htonl(head->bodyLen);
#if 0 /* by june */
	fprintf(stderr, "head->mapType: %d, %d \n", ntohl(head->mapType), ntohl(head->bodyLen));
#endif
	...
	for (i=0, wLen=0; (i<5) && (wLen < buffLen); i++)
	{
		/* write할 수 있는지 확인한다.
		*/
		ret = select (fd+1, NULL, &wFdSet, NULL, &pollTmr);
...
int socklib_addBindSockFdTbl (int port, int fd)
{
	...
	for (i=0; i<SOCKLIB_MAX_BIND_CNT; i++) {
		if (bindSockFdTbl[i].fd == 0) {
			...
			fprintf (stderr, "bindSockFdTbl[i].port, : %d\n", bindSockFdTbl[i].port);
			break;
		}
	}
...
// 07.22 jjinri #define INADDR_NONE	0

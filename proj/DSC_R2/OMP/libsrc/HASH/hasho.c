 ...
 * Hash Library file.
 *
 *		$Id: hasho.c,v 1.1.1.1 2011/04/19 14:13:49 june Exp $
 *
 ...
 **/
stHASHOINFO *
hasho_init(U32 uiShmKey, U16 usKeyLen, U16 usSortKeyLen, U16 usDataLen, U32 uiHashSize, U32 (*func)(void*,U8*), U8 ucResetFlag )
{
...
#ifdef DEBUG
	FPRINTF(LOG_LEVEL,"%s : pstHASHONODE = %p\n",__FUNCTION__, p);
#endif
...
stHASHONODE *
hasho_add_new(stHASHOINFO *pstHASHOINFO, U8 *pKey, U8 *pData, S32 *pRet )
{   
    stHASHONODE *p;
#ifdef DEBUG
    FPRINTF(LOG_LEVEL,"%s : pstHASHONODE = 0x%x\n",__FUNCTION__,(U32) pKey);
		...
		if(*pNodeHead){
			...
			while(pOff){
				...
				FPRINTF(LOG_LEVEL,"##%s : [%3d] p=%p , p offset=%ld\n",
						__FUNCTION__, iIndex, p, (OFFSET) HASHO_OFFSET(pstHASHOINFO, p));
...
main(int argc, char *argv[])
{
	...
	if (argc > 1) {
		if (strcmp (argv [1], "shm") != 0) {
			...
			pstHASHOINFO = hasho_init(0 /**< maim memory */, sizeof(stKEY), sizeof(stKEY), sizeof(stDATA), 4 /**< hash size */, NULL, 0);
		} else {
			fprintf (stderr, "hasho shared memory use!!\n");
			pstHASHOINFO = hasho_init(SHM_KEY_TEST /**< shared memory key */, sizeof(stKEY), sizeof(stKEY), sizeof(stDATA), 4 /**< hash size */, NULL, 0);
		}
	} else 
		pstHASHOINFO = hasho_init(0 /**< maim memory */, sizeof(stKEY), sizeof(stKEY), sizeof(stDATA), 4 /**< hash size */, NULL, 0);
	...
	hasho_find(pstHASHOINFO, (U8 *)&stKey);
 ...
 *     DSC CVS RECOVERY
 *
 *     Revision 1.2  2009/07/22 13:42:44  jjinri
 *     diff -r1.1.1.1 hasho.c
 *     103a104
 *     > #include <strings.h>
 *
 ...
 *     LGT 상암 init
 *
 *     Revision 1.2  2009/04/29 05:31:38  sjjeon
 *     CVS
 *
 ...
 *
 *     Revision 1.2  2009/03/20 09:07:11  jsyoon
 *     hasho_unlink_node 함수의 포인터를 OFFSET으로 변경
 *
 *     Revision 1.1.1.1  2008/12/30 02:32:13  upst_cvs
 *     BSD R3.0.0
 *
 *     Revision 1.2  2008/11/12 05:59:06  upst_cvs
 *     add hasho_add_new
 *
 *     Revision 1.1  2008/10/12 23:15:48  jsyoon
 *     HASH 라이브러리 추가
 *

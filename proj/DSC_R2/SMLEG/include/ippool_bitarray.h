...
#define FFD_NFDBITS			(IPPOOL_BASE*BPBYTE)		/* */
...
#define _IPPOOL_SET(n, p)			p->IPPOOL[n/FFD_NFDBITS] |= (1ul << (n%FFD_NFDBITS))			/* 특정 IP를 1로 셋팅 */
#define _IPPOOL_CLR(n, p)			p->IPPOOL[n/FFD_NFDBITS] &= ~(1ul << (n%FFD_NFDBITS))			/* 특정 IP를 0으로 셋팅 */
#define _IPPOOL_ISSET(n, p)			(p->IPPOOL[n/FFD_NFDBITS] & (1ul<<( n%FFD_NFDBITS))) != 0l	/* 특정 IP가 1인지 확인 */

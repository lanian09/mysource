...
{
//   struct in_addr in;
//   in.s_addr = htonl(key->uiDestIP );
//        fprintf( stderr,"ucProtocol : %d, Reserved: %d, usDestPort:  %d, uiDestIP :  %s \n " ,key->ucProtocol, key->Reserved, key->usDestPort, inet_ntoa(in));
}    
...
{
  //  struct in_addr in;
  //  in.s_addr = htonl(tree->key.uiDestIP);
// fprintf( stderr,"ucProtocol : %d, usDestPort:  %d, uiDestIP :  %s\n" ,tree->key.ucProtocol, tree->key.usDestPort,
// inet_ntoa(in));
...
/*
DESTPORT_TYPE *avl_search_destport_seq( int tindex, short seq)
{
    DESTPORT_TYPE *tree;
...
DESTPORT_TYPE *avl_select_destport( int index, PDESTPORT_KEY first_key, PDESTPORT_KEY last_key)
{
	...
	//&destport_tbl->tuple[stack[top]] 을 써서는 안된다.
	//get_destport함수를 써서, stack[top]==-1 일때는 return 값으로 0이 가도록 한다.

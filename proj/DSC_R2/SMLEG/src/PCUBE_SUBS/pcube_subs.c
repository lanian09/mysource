...
int  Select(SQLHDBC hDbc, long start, long end, char *imsi, char *fileName);
void MainProcess( void *arg_rec, int pro_cnt, int order, char  *imsi, char *fileName );
...
///	long		row_num;
	...
	if( end > start )
	{
		sprintf(lsql_select, " select rows %ld to %ld a.name, b.ip, a.last_update, c.val "
...
printf(" C1[%d] - C2[%s] - C3[%d] - C4[%d] - C5[%s] - "
	...
	*/
			if( select_type == 1 )
			{
				...
				fprintf(fp,"%s\t%s\t%02d-%02d-%02d %02d:%02d:%02d\t%d\n", NAME, IP, LAST_UPDATE.year, LAST_UPDATE.month,
			...
			{
				fprintf(fp,"%d\t%02d-%02d-%02d %02d:%02d:%02d\n ", NAME_INDEX, LAST_UPDATE.year, LAST_UPDATE.month,
...
//	fprintf(fp, "\nCommand terminated successfully\n");
	fprintf(fp, "Command terminated successfully\n");
...
{   
	ARGUMENT arg;
	int	status, i, proc_cnt; 
//	int ret, client_err = 0;
//	int total_tps=0;
//	FILE *fp;
//	char line[10];
	char	imsi[16] = {0,};
...
//	printf("111 : %d\n", now);
	...
	for (i=1 ; i<argc ; i++)
	{
		...
		else if ( memcmp ( argv[i], "-f" , 2 ) == 0 && i != argc - 1 ) {
...
//	printf("222 : %d\n", now);
	...
	MainProcess ( &arg, proc_cnt, 0 ,imsi,fileName);
...
//	printf("333 : %d\n", now);
	...
	for ( i=0;i<proc_cnt;i++){
		if(ret>0){
			...
			MainProcess ( &arg, proc_cnt, i ,imsi,fileName);
...
//			tps = Insert(hDbc, start, end);
		...
		case 'S':
		case 's':
			tps = Select(hDbc, start, end, imsi, fileName);
			break;

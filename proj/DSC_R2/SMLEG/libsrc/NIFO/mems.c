 ...
 *
 *		$Id: mems.c,v 1.2 2011/05/30 00:38:41 jjinri Exp $
 *
...
mems_init(st_MEMSCONF *pstMEMSCONF)
{
	...
	{
		FPRINTF(LOG_BUG, "MEMS_SHM_SIZE =%u uiTotMemSize = %ld\n", (unsigned int)MEMS_SHM_SIZE, uiTotMemSize);
		return NULL;
	}
#if 0
	FPRINTF(LOG_BUG, "NIFO TOTMEMSIZE=%ld MEMS_SHM_SIZE=%u", uiTotMemSize, (unsigned int)MEMS_SHM_SIZE);
#endif
		...
		case MEMS_SHARED_MEM:
            if ((shm_id = shmget (pstMEMSCONF->uiShmKey, MEMS_SHM_SIZE, SEMPERM | IPC_CREAT | IPC_EXCL)) < 0) {
				...
				if (errno == EEXIST) {
                	...
                	if (shm_id < 0) {
						FPRINTF(LOG_BUG,"##%s Shm create fail!!: exit()",__FUNCTION__);
						return NULL;
					}
#if DEBUG
	...
	for(i = 0; i < pstMEMSCONF->uiZoneCnt; i++)
	{
		...
		FPRINTF(LOG_BUG, "ZONE=%d START=%ld FREE=%ld END=%ld\n"
				, pstMEMSZONE->uiZoneID, pstMEMSZONE->offsetNodeStart
				, pstMEMSZONE->offsetFreeList, pstMEMSZONE->offsetNodeEnd);
#endif
	}
...
/** file mems.c
 *     $Log: mems.c,v $
 *     Revision 1.2  2011/05/30 00:38:41  jjinri
 *     *** empty log message ***
 *
 *     Revision 1.1  2011/04/26 09:08:46  jjinri
 *     nifo
 *
 *     Revision 1.3  2011/03/28 06:30:22  jjinri
 *     *** empty log message ***
 *
 *     Revision 1.3  2011/02/22 07:01:12  pkg
 *     pBase 초기화 문제
 *
 *     Revision 1.2  2011/01/13 01:43:39  jjinri
 *     SHM_SIZE printf format %d -> %ld
 *
 *     Revision 1.1.1.1  2011/01/11 03:32:00  uamyd
 *     64bit WNTAM, WGTAM added, 1st source after cvs recovery
 *
 *     Revision 1.3  2010/12/29 06:57:49  swpark
 *     zone matrix add
 *
 *     Revision 1.2  2010/12/23 01:28:30  swpark
 *     nifo zone add
 *

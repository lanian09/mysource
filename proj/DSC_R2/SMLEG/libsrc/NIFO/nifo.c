 ...
 *
 *	$Id: nifo.c,v 1.3 2011/05/30 00:37:27 jjinri Exp $
 *
...
#define	MAX_NIFO_PROCNAME_LEN	10	
    ...
    nifo_node_for_each_start(pMEMSINFO, pNont, &pTmp->nont) {
        ...
        nifo_node_for_each_start(pMEMSINFO, pCont, &pNode->cont) {
			...
			if((MEMS_ID != pMEMSNODEHDR->uiID) || (MEMS_ALLOCED != pMEMSNODEHDR->ucIsFree)) {
				FPRINTF(LOG_BUG, "%s() ALLOC PROCESS[%s] FREE PROCESS[%s] ID[%u][%u]TIME[%u]ISFREE[%d] NONTCNT[%d] CONTCNT[%d]", __FUNCTION__,
 ...
 *  @param *pDbgStr: Process Name (For Debugging)
 *  @param processID: Process ID (For Debugging)
 *  @param *confFile: config File Name ("/DATA/nifo_zone.conf")
 *
 *  @return     stMEMSINFO *    (MEM °u¸®AU Pointer)
 *  @see        nifo.h nifo.c
 *
 *  @exception  
 *  @note       Nothing
...
stMEMSINFO *nifo_init_zone(U8 *pDbgStr, S32 processID, S8 *confFile)
{
	...
	{
		FPRINTF(LOG_BUG, "F=%s:%s.%d nifo_zone_conf_init dRet=%d", __FILE__, __FUNCTION__, __LINE__, dRet);
		return NULL;
	}
	...
	return nifo_init(pMEMSCONF, pDbgStr, processID);
}

/** nifo_conf_print function. 
 *
 *  config print function 
 *
 *  @param st_MEMSCONF: Memory Config Structure
 *
 *  @return     
 *  @see        nifo.h nifo.c
 *
...
//	pstMsgQSub->msgid = 0;
	...
	if((dRet = nifo_node_check(pstMEMSINFO, offset)) < 0) {
		FPRINTF(LOG_BUG, "%s() @@@+++### SEND MSG : CANNOT SEND NON-ALLOCED MEMORY", __FUNCTION__);
//		return -1;
		exit(0);
	}
#endif
...
#if 0 // BY JUNE
		fprintf(stderr, "PROID[%d] mtype[%ld] len[%d] msgqid[%d] offset[%d]\n",
		pstMsgQ->procid, pstMsgQ->mtype, pstMsgQ->len, pstMsgQ->msgqid, *offset);
#endif
#ifdef NIFO_TEST
	if((dRet = nifo_node_check(pstMEMSINFO, *offset)) < 0) {
...
#ifdef JJIN
		FPRINTF(LOG_LEVEL, "%s() @@@+++### ERROR: BROKEN MEMORY !!! will be exit | ALLOC PROCESS[%s] FREE PROCESS[%s] ID[%u]ZONEID[%u]TIME[%u]ISFREE[%d]", __FUNCTION__,
...
/*
 *     $Log: nifo.c,v $
 *     Revision 1.3  2011/05/30 00:37:27  jjinri
 *     *** empty log message ***
 *
 *     Revision 1.2  2011/04/28 05:05:39  jjinri
 *     *** empty log message ***
 *
 *     Revision 1.1  2011/04/26 09:08:46  jjinri
 *     nifo
 *
 *     Revision 1.4  2011/03/28 07:28:24  jjinri
 *     *** empty log message ***
 *
 *     Revision 1.3  2011/03/28 06:30:23  jjinri
 *     *** empty log message ***
 *
 *     Revision 1.3  2011/01/21 02:36:59  swpark
 *     LOG classfy
 *
 *     Revision 1.2  2011/01/11 05:45:24  swpark
 *     config init function config file name parameter add
 *
 *     Revision 1.4  2011/01/06 09:07:10  swpark
 *     config file parameter addINC/cifo.h
 *
 *     Revision 1.3  2010/12/29 06:57:50  swpark
 *     zone matrix add
 *
 *     Revision 1.2  2010/12/23 01:27:34  swpark
 *     nifo zone add
 *

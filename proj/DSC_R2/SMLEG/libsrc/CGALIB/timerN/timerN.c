 ...
 * Timer Library file.
 *
 *		$Id: timerN.c,v 1.1.1.1 2011/04/19 14:13:41 june Exp $
 *
...
#if 0
			fprintf(stderr, "%s : KEY=%d:%u TMR_CNT:%u TMR_DATA=%s\n",
					__FUNCTION__, pstTIMERNKEY->sTimeKey, pstTIMERNKEY->uiTimerNIdIndex
					, pstTIMERNINFO->uiNodeCnt, (unsigned char*)pArg);
#endif
#ifdef DEBUG
...
#endif
#if 0
	fprintf(stderr, "%s : KEY=%d:%u TMR_CNT:%u\n"
			, __FUNCTION__
			, pstTIMERNKEY->sTimeKey, pstTIMERNKEY->uiTimerNIdIndex
			, pstTIMERNINFO->uiNodeCnt);
#endif
...
timerN_draw_all(S8 *filename,S8 *labelname,stTIMERNINFO *pstTIMERNINFO)
{
	...
	for(iIndex=0;iIndex < pstHASHGINFO->uiHashSize;iIndex++){
		...
		for(;p;p=p->next){
			...
			fprintf(fp,"N%p [label = \"{ <n> %p | %3d | <p> %s }\"];\n", p, p,iIndex,s);
		}
	}
	fprintf(fp,"\n\n");
	for(iIndex=0;iIndex < pstHASHGINFO->uiHashSize;iIndex++){
		...
		if(p){
			fprintf(fp,"node0:f%d -> N%p:n;\n",iIndex, p);
		}
		for(;p;p=p->next){
			if(p->next){
				fprintf(fp,"N%p:p -> N%p:n;\n", p , p->next);
			}
		}
	}
	fprintf(fp,"}\n\\enddot\n");
...
timerN_invoke(stTIMERNINFO *pstTIMERNINFO)
{
	...
	while(pstTIMERNINFO->uiCurrentTime <= tt){
        ...
        for( ; ; ) {
#if 0	
			fprintf(stderr, "[timerN_invoke] --------- invoke(p=%p)\n", p);
#endif
            p = (stHASHGNODE *) *(  
                    (OFFSET *) pstHASHGINFO->psthashnode 
			...
			pstTIMERKEY = (stTIMERNKEY *)p->pstKey;
#if 0
			fprintf(stderr, "[timerN_invoke] --------- invoke (TMR_KEY=%d:%u )\n"
					, pstTIMERKEY->sTimeKey, pstTIMERKEY->uiTimerNIdIndex);
#endif
...
void 
invoke_del2(void *pa){
	FPRINTF(LOG_LEVEL, "EXPIRE %s : 0x%x\n", __FUNCTION__,(U32) pa);
}
...
invoke_del1(void *pa){
	stDATA *pstDATA;
	pstDATA = (stDATA *) pa;
	FPRINTF(LOG_LEVEL, "EXPIRE %s : 0x%x  %c %c %c %c\n", __FUNCTION__,(U32) pa
			,pstDATA->a
			,pstDATA->b
			,pstDATA->c
			,pstDATA->d

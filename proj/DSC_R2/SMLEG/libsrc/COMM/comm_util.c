...
void commlib_setupSignals (int *notMaskSig)
{
	...
	for (i=16; i<=SIGMAX; i++)
#else
	/* for (i=16; i<=_NSIG; i++)  For Linux */
	for (i=16; i<=MAXSIG; i++) /* For Solaris */
#endif
	{
...
char *commlib_printTime_usec (void)
{
	...
	if ((pLocalTime = (struct tm*)localtime((time_t*)&now.tv_sec)) == NULL) {
		...
		sprintf (commlib_timeStamp, "%s.%03ld", tmp, (now.tv_usec/1000));
	}
	return ((char*)commlib_timeStamp);
...
char *commlib_printTStamp (void)
{
	...
	if ((pLocalTime = (struct tm*)localtime((time_t*)&now.tv_sec)) == NULL) {
		...
		sprintf(commlib_timeStamp, "%s.%02ld", tmp, (now.tv_usec/10000));
	}

 ...
 *
 *		$Id: mems.c,v 1.1.1.1 2011/04/19 14:13:42 june Exp $
 *
...
mems_init(U32 uiType, U32 uiShmKey, U32 uiSemFlag, U32 uiSemKey, U32 uiHeadRoomSize, U32 uiMemNodeBodySize, U32 uiMemNodeTotCnt)
{
	...
	size_t			uiTotMemSize;	/* 4G가 넘는 메모리가 할당되는 경우를 위해 */
	int				shm_id;

	switch(uiType) {
		case MEMS_MAIN_MEM:
    		uiTotMemSize = (size_t) (stMEMSINFO_SIZE + uiHeadRoomSize 
		...
		case MEMS_SHARED_MEM:
            ...
            if ((shm_id = shmget (uiShmKey, uiTotMemSize, SEMPERM | IPC_CREAT | IPC_EXCL)) < 0) {
				...
				if (errno == EEXIST) {
                	...
                	if (shm_id < 0) {
						FPRINTF(LOG_BUG,"## EEXIST, shmget() FAILED, exit(-1): [SemKey=%d] [Size=%d]\n", 
								uiShmKey, uiTotMemSize);
						exit (-1);
					}

					//FPRINTF(LOG_LEVEL, "EEXIST [SHMKEY=%d] [SHMID=%d]..........\n", uiShmKey, shm_id);
                	if((long)(pstMEMSINFO = (stMEMSINFO *) shmat(shm_id, 0, 0)) == -1)
					{
						FPRINTF(LOG_BUG, "## EEXIST, shmat() FAILED, exit(-1): [shmid=%d]\n", shm_id);
						exit(-1);
					}
					...
					if((uiSemFlag == MEMS_SEMA_ON) && (pstMEMSINFO->uiSemKey == uiSemKey)) {
						...
						FPRINTF(LOG_LEVEL,"## EEXIST, DIFF SemKey, exit(-1): [SemFlag=%d] [SemKey=%d] [uiSemKey=%d]\n",
								uiSemFlag, pstMEMSINFO->uiSemKey, uiSemKey);
						exit (-1);
					}
				}
            }

			//FPRINTF(LOG_LEVEL, "NO EEXIST, [SHMKEY=%d]\n", uiShmKey);
            if((long)(pstMEMSINFO = (stMEMSINFO *) shmat(shm_id, 0, 0)) == -1)
			{
				FPRINTF(LOG_BUG, "## NO EEXIST, shmat() FAILED, exit(-1): [shmid=%d]", shm_id);
				exit(-1);
			}
			/*
			 * pstMEMSINFO의 크기만큼만 초기화 한다.
			 * 멀티 프로세스 환경에서 동시에 큰 공유메모리를 초기화 하는 문제를 없애기 위해 
			 * 메모리의 헤더부분만을 초기화한다. 
			 * 공유메모리를 사용하는 블럭에서 초기화해야 한다. 
			 */
			//memset(pstMEMSINFO, 0, uiTotMemSize);
			memset(pstMEMSINFO, 0, stMEMSINFO_SIZE);
   			pstMEMSINFO->uiTotMemSize = uiTotMemSize;

			if (pstMEMSINFO == NULL) {
				FPRINTF(LOG_LEVEL,"## NO EEXIST, pstMEMSINFO is NULL!!, exit(-1)\n");
				exit (-1);
			}
			break;
		default: 
	...
	pstMEMSINFO->uiReserved1 = 0;					// ADD64
	...
	pstMEMSINFO->uiReserved2 = 0;					// ADD64
...
//	while(pstMEMSINFO->uiMemNodeAllocedCnt < pstMEMSINFO->uiMemNodeTotCnt){
	while(1) {
		...
		if( (pstMEMSINFO->offsetFreeList += (stMEMSNODEHDR_SIZE + pstMEMSINFO->uiMemNodeBodySize)) == pstMEMSINFO->offsetNodeEnd) {
    		pstMEMSINFO->offsetFreeList = pstMEMSINFO->offsetNodeStart;
		}
#ifdef DEBUG
 ...
 *  @see        mems.h
 *
 *  @warning 	만약에 전에 할당한 것에 대해서는 어떻게 할 것인가?  
 * 				free를 누가 시켜주는가 하는 것이다. 
 * 				(일정 시간을 set하여 그것을 넘는 것들이 있을때는 하루에 한번씩 clear를 시킨다든지 하는 작업이 필요할 것이다.) 
 * 				(나름대로의 garbage collection을 수행하여야 한다.)   
 *
 ...
 *     DSC CVS RECOVERY
 *
 *     Revision 1.3  2009/04/27 09:49:54  june
 *     *** empty log message ***
 *
 *     Revision 1.2  2009/04/24 13:48:46  june
 *     *** empty log message ***
 *
 ...
 *
 *     Revision 1.1.1.1  2008/12/30 02:32:00  upst_cvs
 *     BSD R3.0.0
 *
 *     Revision 1.2  2008/10/16 07:31:35  jsyoon
 *     디버그 함수 추가
 *
 *     Revision 1.1  2008/10/12 22:36:40  jsyoon
 *     INIT MIF LIBRARY
 *
 *     Revision 1.1  2008/10/12 22:29:06  jsyoon
 *     *** empty log message ***
 *
 *     Revision 1.1  2008/10/07 23:27:19  jsyoon
 *     *** empty log message ***
 *

...
int 	mmdb_shmid;
extern	OBJECT_TABLE	*object_tbl2;

/**D.1*  Definition of Functions  *************************/
int 	Init_shm_object2( int key );
void 	Init_object2();

int 	Init_PCDROBJ( int key );
int 	Remove_PCDROBJ( int key );
...
int Init_PCDROBJ( int key )
{
    ...
    if ((err = Init_shm_object2( key )) == SHM_ERROR) {
		;
...
int Init_shm_object2( int key )
{
    ...
    if (mmdb_shmid >= 0){ /* 새롭게 shared memory가 생성이 된 경우 */
        ...
        object_tbl2 = (OBJECT_TABLE *)shmat( mmdb_shmid, 0, 0);
        if( object_tbl2 == (OBJECT_TABLE *)-1){
			return SHM_ERROR;
        }       
...
void Init_object2()
{
	...
	for( i=0; i < JMM_OBJECT_RECORD - 1; i++ ) {
		object_tbl2->tuple[i].right = i+1;
	}
	object_tbl2->tuple[i].right = -1;

	object_tbl2->free = 0;
	object_tbl2->root = -1;
	object_tbl2->uiCount = 0;
}

 ...
 *
 *		$Id: memg.c,v 1.3 2011/05/30 00:36:28 jjinri Exp $
 *
 ...
 **/
stMEMGINFO *
memg_init(U32 uiType,U32 uiShmKey,U32 uiHeadRoomSize, U32 uiMemNodeBodySize,U32 uiMemNodeTotCnt, U8 ucResetFlag )
{
	U32				iIndex;
		...
		case MEMG_SHARED_MEM:
            ...
            if ((shm_id = shmget (uiShmKey, uiTotMemSize, SEMPERM | IPC_CREAT | IPC_EXCL)) < 0) {
				...
				if (errno == EEXIST) {
...
#endif
                	if((long)(pstMEMGINFO = (stMEMGINFO *) shmat(shm_id, 0, 0)) == -1)
					{
					...
					if( ucResetFlag == 0 ) 
						return pstMEMGINFO;
					else if( ucResetFlag == 1 ) {
						memset(pstMEMGINFO,0,uiTotMemSize);
						pstMEMGINFO->uiTotMemSize = uiTotMemSize;
					}
					else {
						FPRINTF(LOG_BUG, "RESET PARAMETER ERROR!!");
						exit(-1);
					}
				}
				else {
...
void memg_reset(stMEMGINFO *pstMEMGINFO, size_t base)
{
	U32 iIndex;
	stMEMGNODEHDR *pstMEMGNODEHDR;
	U8 *pBase,*pIndex, *pHeadRoom;

	/* ￠￢C AⓒøA¨o AE¡¾aE¡ⓒ ￥iC¨uuA¡i¨oA */
	pstMEMGINFO->uiMemNodeAllocedCnt = 0;
	pstMEMGINFO->offsetFreeList = pstMEMGINFO->offsetNodeStart;
...
memg_print_all(S8 *pcPrtPrefixStr,stMEMGINFO *pstMEMGINFO)
{
	...
	for(iIndex=0;iIndex < pstMEMGINFO->uiMemNodeTotCnt;iIndex++){
		...
		FPRINTF(LOG_LEVEL,"%s ##2: offset %u 0x%x\n", __FUNCTION__, 
				iIndex * (stMEMGNODEHDR_SIZE + pstMEMGINFO->uiMemNodeBodySize), 
				iIndex * (stMEMGNODEHDR_SIZE + pstMEMGINFO->uiMemNodeBodySize));
...
/** file memg.c
 *     $Log: memg.c,v $
 *     Revision 1.3  2011/05/30 00:36:28  jjinri
 *     *** empty log message ***
 *
 ...
 *
 *     Revision 1.1.1.1  2008/12/30 02:32:12  upst_cvs
 *     BSD R3.0.0
 *

/**		@file	java_main.c
 * 		- JAVA Transaction을 관리 하는 프로세스
 *
 * 		Copyright (c) 2006~ by Upresto Inc. Korea
 * 		All rights reserved
 *
 * 		 $Id: java_main.c,v 1.2 2011/09/05 04:25:52 uamyd Exp $
 *
 * 		@Author		$Author: uamyd $
 * 		@version	$Revision: 1.2 $
 * 		@date		$Date: 2011/09/05 04:25:52 $
 * 		@warning	.
 * 		@ref		java_main.c java_init.c
...
S32 main(S32 argc, S8 *argv[])
{
	...
	/* log_print 초기화 */
	log_init(S_SSHM_LOG_LEVEL, getpid(), SEQ_PROC_A_JAVA, LOG_PATH"/A_JAVA", "A_JAVA");

	/* A_JAVA 초기화 */
	if((dRet = dInitJAVA(&pMEMSINFO)) < 0)
	{
		log_print(LOGN_CRI, "[%s][%s.%d] dInitJAVA dRet[%d]", __FILE__, __FUNCTION__, __LINE__, dRet);
		exit(0);
	}

    if((dRet = set_version(S_SSHM_VERSION, SEQ_PROC_A_JAVA, vERSION)) < 0 ) {
        log_print(LOGN_CRI, "SET_VERSION ERROR(RET=%d,IDX=%d,VER=%s)", dRet, SEQ_PROC_A_JAVA, vERSION);
    }
	log_print(LOGN_CRI, "START JAVA(%s)", vERSION);

	/* MAIN LOOP */
	while(giStopFlag)
	{
		if((offset = gifo_read(pMEMSINFO, gpCIFO, SEQ_PROC_A_JAVA)) > 0) {
...
*/
			...
			if(pLOG_HTTP_TRANS){
				...
				if((dRet = dSend_JAVA_Data(pMEMSINFO, dSeqProcID, pNode)) < 0) {
					log_print(LOGN_CRI, "[%s][%s.%d] MSGQ WRITE FAILE[%d][%s]", __FILE__, __FUNCTION__, __LINE__, dRet, strerror(-dRet));
					break;
				}
			} else {
				log_print(LOGN_CRI,"JAVA : pLOG_HTTP_TRANS is NULL");
				/* Node 삭제 */
 ...
 *  init DQMS2
 *
 *  Revision 1.8  2011/05/09 15:19:49  jsyoon
 *  *** empty log message ***
 *
 ...
 *
 *  Revision 1.6  2009/08/19 12:26:16  pkg
 ...
 *  *** empty log message ***
 *
 *  Revision 1.1.1.1  2009/05/26 02:14:23  dqms
 *  Init TAF_RPPI
 *
 *  Revision 1.2  2008/07/02 07:25:50  dark264sh
 *  *** empty log message ***
 *

...
//	SESSKEY_TBL 	*pstSESSKEYTBL = &stSESSKEYTBL;
	...
	for (i=0; i < MAX_SESSKEY_LIST-1; i++)
	{
		...
		pstSESSKEYTBL->stNode[i].uiNext = 0;
		pstSESSKEYTBL->stNode[i].uiPrev = 0;
		memset(&pstSESSKEYTBL->stNode[i].stSessKey, 0x00, SIP_INFO_KEY_SIZE);
	}
...
int dAddSessKeyNext(pSESSKEY_LIST pstHashStack, pSESSKEY_LIST pstAddStack)
{
	...
	if(!memcmp(&pstStack->stSessKey, &pstAddStack->stSessKey, SIP_INFO_KEY_SIZE))
	{
		log_print(LOGN_CRI, "#### ADDSTACKNEXT::SAME SIP TRANSACTION KEY S1(I=%u P=%u N=%u) S2(I=%u P=%u N=%u)",
				pstStack->uiIndex, pstStack->uiPrev, pstStack->uiNext,
				pstAddStack->uiIndex, pstAddStack->uiPrev, pstAddStack->uiNext);
		return -2;
	}

	pstAddStack->uiPrev = pstStack->uiIndex;
	pstAddStack->uiNext = pstStack->uiNext;

	if(pstStack->uiNext > 0) {
		if(pstStackTbl->stNode[pstStack->uiNext].uiPrev == pstStack->uiIndex)
			pstStackTbl->stNode[pstStack->uiNext].uiPrev = pstAddStack->uiIndex;
		else
	...
	pstStack->uiNext = pstAddStack->uiIndex;

	log_print(LOGN_DEBUG, "#### ADDSTACKNEXT OLD: %d OLD->NEXT: %d NEW: %d NEW->NEXT: %d",
			pstStack->uiIndex, pstStack->uiNext, pstAddStack->uiIndex, pstAddStack->uiNext);
...
void FreeSessKeyList(pSESSKEY_LIST node)
{
	...
	node->uiNext = 0;
	node->uiPrev = 0;
	memset(&node->stSessKey, 0x00, SIP_INFO_KEY_SIZE);
...
int dDelSessKeyList(pSESSKEY_LIST pstDelStack, stHASHOINFO *pSIPHASH, stMEMSINFO *pMEMSINFO, UINT uiLastLogTime)
{
	UINT 			uiIndex;
	pSESSKEY_TBL 	pstStackTbl = pstSESSKEYTBL;
	pSESSKEY_LIST 	pstStack;
...
//	pstStack = pstDelStack;
	...
	while(uiIndex != 0) {
		...
		log_print(LOGN_DEBUG, "#### WHILE SESSCNT: %d DEL->NEXT: %d INDEX: %d INDEX: %d NEXT: %d", 
				dSessCnt, pstDelStack->uiNext, uiIndex, pstStack->uiIndex, pstStack->uiNext );
		if(pstStack->uiNext > 0) {
			uiIndex = pstStack->uiNext;
		} else 
			break;
	}
	...
	for(i=0; i<dSessCnt && uiIndex > 0; i++) {
		pstStack = &pstStackTbl->stNode[uiIndex];
		uiIndex = pstStack->uiPrev;

		if(uiLastLogTime == 0 || pstStack->SessStartTime < uiLastLogTime) {
			if(pstStack->uiPrev > 0) {
				if(pstStackTbl->stNode[pstStack->uiPrev].uiNext == pstStack->uiIndex)
					pstStackTbl->stNode[pstStack->uiPrev].uiNext = pstStack->uiNext;
				else
					log_print(LOGN_WARN, "#### ERROR : STACKDEL Linked List Corrupted Node Index=%u Prev=%u Next=%u NextPrev=%u",
							pstStack->uiIndex, pstStack->uiPrev, pstStack->uiNext,
							pstStackTbl->stNode[pstStack->uiPrev].uiNext);
			} else {
				log_print(LOGN_DEBUG, "#### STACKDEL Linked List Node First Index=%u Prev=%u Next=%u i=%d",
						pstStack->uiIndex, pstStack->uiPrev, pstStack->uiNext, i);
			}
			if(pstStack->uiNext > 0) {
				if(pstStackTbl->stNode[pstStack->uiNext].uiPrev == pstStack->uiIndex)
					pstStackTbl->stNode[pstStack->uiNext].uiPrev = pstStack->uiPrev;
				else
					log_print(LOGN_WARN, "#### ERROR : STACKDEL Linked List Corrupted Node Index=%u Prev=%u Next=%u NextPrev=%u",
							pstStack->uiIndex, pstStack->uiPrev, pstStack->uiNext,
							pstStackTbl->stNode[pstStack->uiNext].uiPrev);
			} else {
				log_print(LOGN_DEBUG, "#### STACKDEL Linked List Node First Index=%u Prev=%u Next=%u i=%d",
				pstStack->uiIndex, pstStack->uiPrev, pstStack->uiNext, i);
			}

			if((pHASHNODE = hasho_find(pSIPHASH, (U8 *)&pstStack->stSessKey)) != NULL) {

				pSIPINFO = (SIP_INFO *)nifo_ptr(pSIPHASH, pHASHNODE->offset_Data);
				dCloseSIPTrans(pMEMSINFO, pSIPHASH, &pstStack->stSessKey, pSIPINFO);

				log_print(LOGN_DEBUG, "#### dCloseSession CallID=%s CSeq=%u FromTag=%s", 
						pstStack->stSessKey.CallID, pstStack->stSessKey.CSeq, pstStack->stSessKey.FromTag);
			} else {
				log_print(LOGN_DEBUG, "#### NOT FOUND HASH CallID=%s CSeq=%u FromTag=%s",
						pstStack->stSessKey.CallID, pstStack->stSessKey.CSeq, pstStack->stSessKey.FromTag);
			}
			log_print(LOGN_DEBUG, "#### OLD: %d OLD->NEXT: %d old->next: %d old->prev: %d", 
					pstStack->uiIndex, pstStack->uiNext, pstStack->next, pstStack->prev);

			if((i == dSessCnt - 1) && (pstStack->uiNext > 0)) {
				pSESSKEY_LIST   pstHeadStack;
				pstHeadStack = &pstStackTbl->stNode[pstStack->uiNext];
			...
			dCurrSessCnt++;
			log_print(LOGN_DEBUG, "#### TIME NOT MATCH HASH STIME:%u LTIME:%u CallID=%s CSeq=%u FromTag=%s CNT:%d", 
...
int dDelSessKeyNext(pSESSKEY_LIST pstDelStack, SIP_INFO_KEY *pSIPINFOKEY)
{
	UINT 			uiIndex;
	pSESSKEY_TBL 	pstStackTbl = pstSESSKEYTBL;
	pSESSKEY_LIST 	pstStack;
	...
	for(i=0; i<dSessCnt && uiIndex > 0; i++) {
		...
		if(!memcmp(&pstStack->stSessKey, pSIPINFOKEY, SIP_INFO_KEY_SIZE)) {
			log_print(LOGN_DEBUG, "#### FOUND SAME INFOKEY CallID=%s CSeq=%u FromTag=%s",
					pstStack->stSessKey.CallID, pstStack->stSessKey.CSeq, pstStack->stSessKey.FromTag);
			...
			dCurrSessCnt++;
			log_print(LOGN_DEBUG, "#### SIPINFOKEY NOT MATCH CallID=%s CSeq=%u FromTag=%s CNT:%d", 
					pstStack->stSessKey.CallID, pstStack->stSessKey.CSeq, pstStack->stSessKey.FromTag,
					dCurrSessCnt);
		}
	}

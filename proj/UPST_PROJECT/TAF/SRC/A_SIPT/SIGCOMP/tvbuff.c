 ...
 * 		other tvbuffs.
 *
 * $Id: tvbuff.c,v 1.1.1.1 2011/08/29 05:56:42 dcham Exp $
 *
...
void
tvb_set_child_real_data_tvbuff(char* parent, char* child)
{
#if 0
...
gint
tvb_reported_length_remaining(gchar *tvb, gint tlen, gint offset)
{
    return tlen - offset;
}

static const guint8*
ensure_contiguous(gchar *tvb, gint tlen, gint offset, gint length)
{
//    int exception;
    const guint8 *p;

    if (tlen < offset+length) return NULL;
    p = (guint8*)(tvb + offset);
...
guint16
tvb_get_ntohs(gchar *tvb, gint tlen, gint offset)
{
	const guint8* ptr;

	ptr = ensure_contiguous(tvb, tlen, offset, sizeof(guint16));

	return pntohs(ptr);
}

guint8
tvb_get_guint8(gchar *tvb, gint tlen, gint offset)
{
	const guint8* ptr;
    
	ptr = ensure_contiguous(tvb, tlen, offset, sizeof(guint8));
	return *ptr;
}

void
tvb_set_free_cb(char* tvb)
{
#if 0
...
guint
tvb_length(gchar* tvb, gint tlen)
{
	return tlen;
}

guint8*
tvb_memcpy(gchar *tvb, gint tlen, guint8* target, gint offset, gint length)
{
    return (guint8*) memcpy(target, tvb + offset, length);
}

GByteArray*
tvb_new_subset(GByteArray *backing, gint backing_offset, gint backing_length, gint reported_length)
{
#if 0

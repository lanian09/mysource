...
ULONG read_one_packet(void *buffer, ULONG bufferlen, int dDagNum )
{
    ...
    if( bufferlen < sizeof(dag_record_t)-1 ) {
		log_print( LOGN_CRI, "CHECK BUFFLEN[%d] DAGRECLEN[%d]", bufferlen, sizeof(dag_record_t)-1);
        return 0;
	}
...
//	T_CAPHDR        *pstCAPHead;
    ...
    //if((ret = Send_CAPD_Data(pstMEMSINFO, myqid, pstNode, pstCAPHead->curtime)) < 0) {
    if((ret = Send_CAPD_Data(pstMEMSINFO, pstNode, pstCAPHead->curtime)) < 0) {
        log_print( LOGN_CRI, "[%s.%d] Send_CAPD_Data [%d][%s]", __FUNCTION__, __LINE__, ret, strerror(-ret));
		nifo_node_delete(pstMEMSINFO, pstNode);
		return -4;
    }

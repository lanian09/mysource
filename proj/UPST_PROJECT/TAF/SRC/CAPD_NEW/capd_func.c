...
int open_device(char *dagname_buf)
{
	...
	if (-1 == dag_parse_name(dagname_buf, dagname, DAGNAME_BUFSIZE, &dag_devnum)) {
		dAppLog(LOGN_CRI, "%s: FAIL[dag_parse_name] [%s]", __FUNCTION__, strerror(errno));
		exit(errno);
	}

	if((dag_fd = dag_open(dagname)) < 0) {
		dAppLog(LOGN_CRI, "%s: FAIL[dag_open] [%s]", __FUNCTION__, strerror(errno));
		exit(errno);
	}
	...
	if(dag_configure(dag_fd, buffer) < 0) {
		dAppLog(LOGN_CRI, "%s: FAIL[dag_configure] [%s]", __FUNCTION__, strerror(errno));
		exit(errno);
	}

	if(dag_attach_stream(dag_fd, dag_devnum, 0, 0) < 0) {
		dAppLog(LOGN_CRI, "%s: FAIL[dag_attach_stream] [%s]", __FUNCTION__, strerror(errno));
		exit(errno);
	}

	if(dag_start_stream(dag_fd, dag_devnum) < 0) {
		dAppLog(LOGN_CRI, "%s: FAIL[dag_start_stream] [%s]", __FUNCTION__, strerror(errno));
		exit(errno);
	}
	...
	dAppLog(LOGN_INFO, "DEVICE[%04x]: %s [FCS: %d]",
	...
	dAppLog( LOGN_DEBUG, "dag_devnum:%d", dag_devnum );
...
//    int 				ret;
    ...
    while( loop_continue )
    {
		...
		if( chkTime != curTime ) {
			...
			if( stPortStatus[0].uiLastCnt == stPortStatus[0].uiCurrCnt ) {
				...
				if( stPortStatus[0].uiRetryCnt > 2 ) {
					...
					if( fidb->mirrorActsts[0] == DEF_ACTIVE && fidb->mirrorsts[1] != CRITICAL ) {
						...
						dAppLog( LOGN_CRI, "CHANGE ACTIVE 0 -> 1..");
					}
				}
			}
			else {
			...
			if( stPortStatus[1].uiLastCnt == stPortStatus[1].uiCurrCnt ) {
                ...
                if( stPortStatus[1].uiRetryCnt > 2 ) {
                    ...
                    if( fidb->mirrorActsts[1] == DEF_ACTIVE && fidb->mirrorsts[0] != CRITICAL ) {
						...
						dAppLog( LOGN_CRI, "CHANGE ACTIVE 1 -> 0..");
                    }
                }
            }
            else {
			...
			dAppLog( LOGN_DEBUG, "[0] STS:0X%02X ACT:%u [1] STS:0X%02X ACT:%u",
...
ULONG read_one_packet(void *buffer, ULONG bufferlen, int dDagNum )
{
    ...
    if( bufferlen < sizeof(dag_record_t)-1 ) {
		dAppLog( LOGN_CRI, "CHECK BUFFLEN[%d] DAGRECLEN[%d]", bufferlen, sizeof(dag_record_t)-1);
        return 0;
	}
#endif
    ...
    switch( cap_rec->type )
    {
        ...
        case TYPE_COLOR_ETH:
            dAppLog(LOGN_INFO, "NO NEED TO HANDLE: TYPE[%d]", cap_rec->type);
            break;

        default:
            dAppLog(LOGN_WARN, "INVALID TYPE CHECK: TYPE[%d]", cap_rec->type);
            break;
    }
...
int handle_ethernet(dag_record_t *cap_rec, int dDagNum )
{
    ...
    */
    ...
    dAppLog(LOGN_INFO, "DAG_NUM[%02d] RLEN[%d] ETH_LEN[%d] CAPLEN[%d] CAPTIME[%ld.%06ld] PORT[%d]",
	...
	if( dRet < 0 ) {
		dAppLog( LOGN_INFO, LH"ERROR IN do_action dRet:%d", LT, dRet);
		return -1;
	}
...
int dump_DebugString(char *debug_str, char *s, int len)
{
	...
	dAppLog(LOGN_DEBUG,"### %s",debug_str);
	p =(unsigned char *) s;
	for(line = 1; len > 0; len -= WIDTH,line++) {
		...
		dAppLog(LOGN_DEBUG,"%04x: %-*s    %s",line - 1,WIDTH * 3,lbuf,rbuf);
	}
	return line;
}
...
//	T_CAPHDR        *pstCAPHead;
    ...
    if((ret = dSend_CAPD_Data(pstMEMSINFO, pstNode, pstCAPHead->curtime)) < 0) {
        dAppLog( LOGN_CRI, LH"dSend_CAPD_Data [%d][%s]", LT, ret, strerror(-ret));
		nifo_node_delete(pstMEMSINFO, pstNode);
		return -4;
    }
...
int dSend_CAPD_Data(stMEMSINFO *pstMEMSINFO, U8 *pNode, U32 sec)
{
    ...
    if(Send_Node_Cnt > Collection_Cnt)
	{
		...
		else
		{
			if(pNode == NULL)
			{
				log_print(LOGN_CRI, LH"### Send Buffered Packet[%d]", LT, Send_Node_Cnt);				
			}
    ...
    if( (old_time + COLLECTION_TIME) < sec) {
        ...
        dAppLog(LOGN_INFO, "CHECK_PKT:%d COLLECTION_CNT:%d", check_pkt, Collection_Cnt);
        check_pkt = 0;
        old_time = sec;
    }
#endif /* BUFFERING */

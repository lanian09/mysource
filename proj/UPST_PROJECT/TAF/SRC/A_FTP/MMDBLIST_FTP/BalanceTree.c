...
int avl_insert_tcp( PSESS_KEY key, long long *body, int *root )
{
	...
	while(Pi != -1){  /*  min를 인서트할 T를 찾는다 */
        ...
        if( memcmp( key, &P->key, SESS_KEY_SIZE ) < 0 ) {  
            Q = P;  /* Q는 P의 부모노드가 된다 */
			Pi = P->left;
        }
        else if( memcmp( key, &P->key, SESS_KEY_SIZE ) > 0 ) {  
            Q = P;
			Pi = P->right;
        }
        else 
	...
	insert = get_tcp(index);
	for (i = 0; i < MAX_SESS_BODY_LEN;i++) {
	...
	else if( memcmp( key, &Q->key, SESS_KEY_SIZE ) < 0 )
	...
	A = get_tcp(Ai);
    if( memcmp( key, &A->key, SESS_KEY_SIZE ) > 0 ){  
        P = get_tcp(A->right);
        B = P;
		Bi = A->right;
        d = RIGHTSIDE;
...
{
	SESS_TYPE *grand_child, *child, *parent;
	SESS_TYPE *node;
	int left, right;
	...
	if (child->bf == LEFTSIDE) {
	 ...
	 /* 윗단의 노드에도 영향이 있음 */
		...
		node->bf = 0;
   		*pindex = left;
		parent = &pstSessTbl->tuple[*pindex];
  		parent->bf = 0;
		return 1;
	 ...
	 /* 윗단의 노드에도 영향이 있음 */
	 	right = child->right;
  	    grand_child = &pstSessTbl->tuple[right];
...
int right_rotation_tcp( int index, int *pindex )
{
	SESS_TYPE *grand_child, *child, *parent;
	SESS_TYPE *node;	
	int left, right;
	
	node = &pstSessTbl->tuple[index];
	right = node->right;
	child = &pstSessTbl->tuple[right];
	...
	/* 윗단의 노드에도 영향이 있음 */
	 ...
	 /* 윗단의 노드에도 영향이 있음 */
	 	left = child->left;
  	    grand_child = &pstSessTbl->tuple[left];
...
SESS_TYPE *avl_search_tcp( int tindex, PSESS_KEY key )
{
	SESS_TYPE *tree;

	while(tindex != -1) {
		tree = &pstSessTbl->tuple[tindex];
    	if( memcmp( key, &tree->key, SESS_KEY_SIZE ) < 0 )
			tindex = tree->left;
    	else if( memcmp( key, &tree->key, SESS_KEY_SIZE ) > 0 )
			tindex = tree->right;
		else
			return tree;
	}
...
int avl_delete_tcp( int *root, PSESS_KEY delkey )
{
	SESS_TYPE *thenode, *left_node, *right_node;
	SESS_TYPE *pre_delnode, *delnode, *rotate;
	...
	while(dindex != -1){
		...
		stack[top++] = dindex;
		delnode = get_tcp(dindex);
   		if( memcmp( delkey, &delnode->key, SESS_KEY_SIZE ) < 0 ){  
			pre_delnode = delnode;
            dindex = delnode->left;
   		} 
		else if( memcmp( delkey, &delnode->key, SESS_KEY_SIZE ) > 0 ){  
			pre_delnode = delnode;
	...
	if(delnode->left == -1 && delnode->right == -1){
		...
		else if( memcmp( delkey, &pre_delnode->key, SESS_KEY_SIZE ) > 0 ){  
	...
	/* 지울 노드가 2개의 sub Tree를 가지는 경우*/
		...
		/* 좌측 트리중 가장 큰 값을 찾는다 */
		left_node = get_tcp(del_left);
		right_node = get_tcp(del_right);

		if(left_node->right != -1){
			while(left_node->right != -1){
				pre_delnode = get_tcp(del_left);
                stack[top++] = del_left;
		...
		/* 윗단에 영향을 주지 않으므로 바로 return 한다*/
			...
			/* bf문제를 고려해주어야 한다.*/
			thenode = get_tcp(*root);

			if (pre_delnode == 0) {
				if (thenode->bf >= 0){
      	 	 		thenode->key = (&pstSessTbl->tuple[del_left])->key;
					for (i = 0;i < MAX_SESS_BODY_LEN;i++)
				...
				if (delnode->bf > 0){
   				...
   				if( memcmp( delkey, &pre_delnode->key, SESS_KEY_SIZE ) > 0 )
					side = RIGHTSIDE;
				else 
					side = LEFTSIDE;
				top--;
    ...
    while(top > 1){
		rindex = stack[--top];
		rotate = get_tcp(rindex);
		pre_delnode = get_tcp(stack[top-1]);	/* Parent */
		
		if (side < 0) {
            switch (rotate->bf) {
                case LEFTSIDE:
					if(pre_delnode == 0){
   	                	if(left_rotation_tcp(rindex, root) == 0)
							return 0;
						else
						    break;
					}
					else if(pre_delnode->right == rindex){
   	                	if(left_rotation_tcp(rindex, &pre_delnode->right) == 0)
							return 0;
						else
						    break;
					} 
					else if(pre_delnode->left == rindex){
   	                	if(left_rotation_tcp(rindex, &pre_delnode->left) == 0)
							return 0;
						else
						    break;
					}
			...
			side = memcmp( &pre_delnode->key, &rotate->key, SESS_KEY_SIZE );
            ...
            switch (rotate->bf) {
                ...
                case RIGHTSIDE: 
                    if(pre_delnode == 0){
                        if(right_rotation_tcp(rindex, root) == 0)
                            return 0;
                        else
                            break;
                    } 
					else if(pre_delnode->right == rindex){
                        if(right_rotation_tcp(rindex, &pre_delnode->right) == 0)
                            return 0;
                        else
                            break;
                    } 
					else if (pre_delnode->left == rindex){
                        if(right_rotation_tcp(rindex, &pre_delnode->left) == 0)
                            return 0;
                        else
                            break;
					}
...
******/
SESS_TYPE *avl_select_tcp( int index, PSESS_KEY first_key, PSESS_KEY last_key)
{
	SESS_TYPE *tree;
    int stack[100];
    int top;

    if( index == -1 || memcmp(first_key, last_key, SESS_KEY_SIZE) >= 0)
        return 0;
    ...
    while(index != -1)
    {
		tree = &pstSessTbl->tuple[index];
        if (memcmp(first_key, &tree->key, SESS_KEY_SIZE) < 0)
        {
            stack[top++] = index;
            index = tree->left;
        }
        else if (memcmp(first_key, &tree->key, SESS_KEY_SIZE) > 0)
        {
            index = tree->right;
        }
        else
	...
	if (stack[--top] != -1)
	{
		tree = &pstSessTbl->tuple[stack[top]];
    	if (memcmp(&tree->key, last_key, SESS_KEY_SIZE) >= 0)
        	return 0;
	}

	//여기서는 stack[top]==-1 인 경우도 고려해야 하므로
	//&pstSessTbl->tuple[stack[top]] 을 써서는 안된다.
	//get_tcp함수를 써서, stack[top]==-1 일때는 return 값으로 0이 가도록 한다.
	tree = get_tcp( stack[top] );
    return tree;
}
...
{
	if( index >= 0 && index < MAX_SESS_RECORD )
		return (&pstSessTbl->tuple[index]);
	else
		return 0;
}

...
extern int gdMyQid;
extern int gdIFQid;
extern int gdCondQid;
extern int gdFSTATQid;
extern int gdIndex;
...
int dIsRcvedMessage(st_MsgQ *pstMsg)
{
	...
	if( (dRet = dMsgrcv(&pstMsg)) < 0 ){
		if( dRet != -1 ){
			log_print(LOGN_CRI, "%s.%d:%s FAILED IN dMsgrcv(ALMD)",__FILE__,__LINE__,__FUNCTION__);
		}
		return -2;
	}
...
int dSend_FSTAT(pst_MsgQ pstMsg, char *data)
{
	...
	if( (dRet = dGetNode(&pNODE, &pstMsg)) < 0 ){
		log_print(LOGN_CRI,"%s.%d:%s FAILED IN dGetNode(MOND), errno=%d:%s",
	...
	pstMsg->dMsgQID		= 0;
	pstMsg->ucProID		= SEQ_PROC_MOND;
	pstMsg->usRetCode	= 0;
	pstMsg->llIndex		= gdIndex++;

	memcpy(pstMsg->szBody, data, pstMsg->usBodyLen);

	dLen = DEF_MSGHEAD_LEN + pstMsg->usBodyLen - sizeof(long);
...
void Send_CondMess(int systype, int sysno, int loctype, char invtype, short invno, char almstatus, char oldalm, long long llLoadVal )
{
    ...
    log_print(LOGN_DEBUG,"[SUCCESS] MSGSND COND TYPE=%d,SID=%d,MID=%d,BODY=%d,TAFID=%d,TAMID=%d",
        pstSndSub->usType, pstSndSub->usSvcID, pstSndSub->usMsgID,
        pstSndMsg->usBodyLen, pstSndMsg->ucNTAFID, pstSndMsg->ucNTAMID );
}

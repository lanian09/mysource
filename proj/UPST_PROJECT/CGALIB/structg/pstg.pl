#!/usr/bin/perl
##
## $Id: pstg.pl,v 1.1.1.1 2011/08/29 05:56:44 dcham Exp $
##
	...
	if($in_typedef == 2) {
		...
		if($typedef_prt =~ /\s*\<\s*STG_COMMON\s*\:\s*(\w+)\s*\>/){
			...
			foreach $temp (@temp){
				...
				$stg_common_association .= "$stg_var:$stg_ass;  $stg_checking_value   /**< Reference : $common_name */  /**< $stg_parsing */\n";
				$stg_common_text_parsing .= "$stg_var;  $stg_parsing   $stg_checking_value   /**< Reference : $common_name */  /**< $stg_ass */\n";
				$stg_common_normal .= "$stg_var;  $stg_checking_value  /**< Reference : $common_name */    /**< $stg_parsing $stg_ass */\n";
			}
			...
			if($typedef_prt =~ /STG_LOG_TEXT_PARSING/){
				$typedef_prt =~ s/\t?\<\s*STG_COMMON\s*\:\s*(\w+)\s*\>/$stg_common_text_parsing/g;
				print DBG "STG_COMMON = $stg_common_text_parsing\n";
			} elsif($typedef_prt =~ /STG_ASSOCIATION/){
				$typedef_prt =~ s/\t?\<\s*STG_COMMON\s*\:\s*(\w+)\s*\>/$stg_common_association/g;
				print DBG "STG_COMMON = $stg_common_text_parsing\n";
			} else {
				print DBG "STG_COMMON = $stg_common_normal\n";
				$typedef_prt =~ s/\t?\<\s*STG_COMMON\s*\:\s*(\w+)\s*\>/$stg_common_normal/g;
			}

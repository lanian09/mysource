#!/usr/bin/perl
##
## $Id: structg.pl,v 1.1.1.1 2011/08/29 05:56:44 dcham Exp $
##
...
$type{"DEF"} = "long";
$type_size{"DEF"} = "sizeof(long)";
$type_enc_func{"DEF"} = "ntohl";
$type_printV{"DEF"} = "\%ld";
$type_printM{"DEF"} = "(long)";
$type_define{"DEF"} = "long";
$type_CREATE{"DEF"} = "NUMBER";
$type_set_func{"DEF"} = "atol";
$type_set_type{"DEF"} = "STG_DEF";
...
$type_printPre{"IP4"} = "S8 STG_PrintPre[1024]\; S8 tmp[1024]\; sprintf(STG_PrintPre, \"\%d\.\%d\.\%d\.\%d\", HIPADDR(+IP4+))\; sprintf(tmp, \"\\t\%u\", +IP4+)\; strcat(STG_PrintPre, tmp)\;";
#$type_printV{"IP4"} = "\%d\.\%d\.\%d\.\%d";
$type_printV{"IP4"} = "\%s";
#$type_printM{"IP4"} = "HIPADDR";
$type_printM{"IP4"} = "STG_PrintPre";
...
#$type_printV{"OFFSET"} = "\%d";
#$type_define{"OFFSET"} = "int";
#$type_CREATE{"OFFSET"} = "NUMBER";
#$type_set_type{"OFFSET"} = "STG_INTEGER";
$type{"OFFSET"} = "long";
$type_size{"OFFSET"} = "sizeof(long)";
$type_enc_func{"OFFSET"} = "ntohl";
$type_printV{"OFFSET"} = "\%ld";
$type_define{"OFFSET"} = "long";
$type_CREATE{"OFFSET"} = "NUMBER";
$type_set_type{"OFFSET"} = "STG_INTEGER";
### STIME : 초 시간  (time_t)
$type{"STIME"} = "time_t";			
...
$type_printPre{"STIME"} = "S8 STG_PrintPre[1024]\; S8 tmp[1024]\; time_t ttCallTime = +STIME+\; if(+STIME+){ strftime(STG_PrintPre, 1024, \"\%Y\%m\%d\%H\%M\%S\", localtime((time_t *)&ttCallTime))\; sprintf(tmp, \"\\t\%u\", +STIME+)\; strcat(STG_PrintPre, tmp)\; } else { sprintf(STG_PrintPre,\"0\\t0\"); }";
...
sub Ichange
{
	...
	foreach $temp (keys %$stcI_for){
		print_fp("STCI tmp OUTPUT FileName :  $temp\n",STDOUT,DBG);
		open(STCI_OUTPUT , ">$outputdir/tmp/$temp\.stc");
		print STCI_OUTPUT "FileName : $temp\.$stcI_extension\n";
		print "FileName : $temp\.l\n";
		@stcI_lines = split("\n",$stcI_lines);
...
#print "A2 $ttt\n";
		...
		close(STCI_OUTPUT);
		Cchange("$outputdir/tmp/$temp\.stc","","");			# OUTPUT안에 저장하고, makefile에도 넣어라.
	}
	close(STCI_INPUT);
}
...
sub Cchange
{
	...
	while ($in = <INPUTC>) {
		if($in =~ /^FileName\s*\:\s*(\S+)\s*$/){
			...
			$stg_stc_file{$1} = $1;
			print_fp("STC FileName : stg_stc_file : $stg_stc_file{$1} \n",STDOUT,DBG);
			open(OUTPUTC , ">$outputdir/$stc_output_dir/$stc_filename_output");
			if($stc_filename_output =~ /\.l\s*$/){	
				#NONE#
				# .l일때는 처음에 include를 선언하면 error가 발생한다.
			} else {
				print OUTPUTC "\n#include \"$FileName\"\n\n\n";		# .c안에서는 되는데 .l에서 문제가 발생함.
...
#print DBG "Set Hash 11 : $iterate_lines\n";
				...
				my $ifequal_action="";
				my $is_ifequal = 0;
#print DBG "Cchange IFEQUAL NotIFEQUAL<<\n$iterate_lines\n>>\n";
				foreach $temp (@temp){
					if($temp =~ /^\s*IFEQUAL\s*\(\s*([^ \t\{\}\,\(\)#]*)\s*\,\s*([^ \t\{\}\,\(\)#]*)\s*\)\s*#\{(.*)/){
					...
					} elsif($temp =~ /^\s*NotIFEQUAL\s*\(\s*([^ \t\{\}\,\(\)#]*)\s*\,\s*([^ \t\{\}\,\(\)#]*)\s*\)\s*#\{(.*)/){
						$ifequal_start = 1;
						$ifequal_one = $1;
						$ifequal_two = $2;
						$ifequal_action = $3;
						$is_ifequal = 0;
						print DBG "NotIFEQUAL 1>>> $ifequal_start [$ifequal_one] [$ifequal_two] $ifequal_action\n";
						...
						$ifequal_two = $2;
						$ifequal_action = $3;
						print DBG "NotIFEQUAL 0>>> $ifequal_start [$ifequal_one] [$ifequal_two] $ifequal_action\n";
						if($ifequal_one ne $ifequal_two){	# 다르면  NotIFEQUAL이므로 처리
							print DBG "NotIFEQUAL 0.5>>> $ifequal_start [$ifequal_one] [$ifequal_two] $ifequal_action\n";
							print_fp("$ifequal_action\n" , OUTPUTC,DBG);
						} else {
							# NONE
						}
						...
						if($ifequal_start == 0){
							...
							if( ($is_ifequal == 1) && ($ifequal_one eq $ifequal_two) ){	# 같으면 IFEQUAL이므로 처리
								...
								print_fp("$ifequal_action\n" , OUTPUTC,DBG);
							} elsif( ($is_ifequal == 0) && ($ifequal_one ne $ifequal_two) ){	# 같으면 IFEQUAL이므로 처리
								print DBG "NotIFEQUAL 3.7>>> $ifequal_action\n";
								print_fp("$ifequal_action\n" , OUTPUTC,DBG);
							} else {
								# NONE
							}
...
#print DBG "C : Iterator_recursion : \$iterate_lines = $iterate_lines ]]]\n";
	...
	if($iterate_var_type eq "\%"){
		...
		print DBG "--> ARRAY : $iterate_var_name  size =  $my_cnt\n";
		for(my $i = 0 ; $i < $my_cnt ; $i++){
			print DBG "array : \$$iterate_var_name \[ $i \] = $$iterate_var_name[$i]\n";
			$temp = $iterate_lines;
...
sub replace_var_with_value
{
	...
	$in_cnt = 0;
	while(
		($replace_in =~ /Set\s*\:\s*(\w+)\s*\{\s*(\w+)\s*\}\s*\=\s*\"(.*)\"\s*\n/)   # SET HASH
	){
		$in_cnt ++;
		if($replace_in =~ s/Set\s*\:\s*(\w+)\s*\{\s*(\w+)\s*\}\s*\=\s*\"(.*)\"\s*\n//){   # SET HASH
			$$1{$2} = $3;
...
sub save_typedef
{
	...
	open(ENC , ">$outputdir/$typedef_enc_func{$typedef_name}" . ".c");
	$filelist{"$typedef_enc_func{$typedef_name}" . ".c"} = "CFILE";
			open(UPR,"<header.upr");
			while($upr = <UPR>){
				$upr =~ s/\+FileName\+/$typedef_enc_func{$typedef_name}.c/;
				$upr =~ s/\+Warning\+/\$type???로 정의된 것들만 사용가능/;
				$upr =~ s/\+ToDo\+/library를 만들기 위한 Makefile을 만들자/;
				$upr =~ s/\+Intro\+/typedef를 위한 functions/;
	...
	open(DEC , ">$outputdir/$typedef_dec_func{$typedef_name}" . ".c");
	$filelist{"$typedef_dec_func{$typedef_name}" . ".c"} = "CFILE";
			open(UPR,"<header.upr");
			while($upr = <UPR>){
				$upr =~ s/\+FileName\+/$typedef_dec_func{$typedef_name}.c/;
				$upr =~ s/\+Warning\+/\$type???로 정의된 것들만 사용가능/;
				$upr =~ s/\+ToDo\+/library를 만들기 위한 Makefile을 만들자/;
				$upr =~ s/\+Intro\+/typedef를 위한 functions/;
				$upr =~ s/\+Requirement\+/규칙에 틀린 곳을 찾아주세요./;
	...
	open(PRT , ">$outputdir/$typedef_prt_func{$typedef_name}" . ".c");
	open(FPP , ">$outputdir/$typedef_fpp_func{$typedef_name}" . ".c");
	$filelist{"$typedef_prt_func{$typedef_name}" . ".c"} = "CFILE";
	$filelist{"$typedef_fpp_func{$typedef_name}" . ".c"} = "CFILE";
			open(UPR,"<header.upr");
			while($upr = <UPR>){
				$upr =~ s/\+FileName\+/$typedef_prt_func{$typedef_name}.c/;
				$upr =~ s/\+Warning\+/\$type???로 정의된 것들만 사용가능/;
				$upr =~ s/\+ToDo\+/library를 만들기 위한 Makefile을 만들자/;
				$upr =~ s/\+Intro\+/typedef를 위한 functions/;
				$upr =~ s/\+Requirement\+/규칙에 틀린 곳을 찾아주세요./;
			...
			open(UPR,"<header.upr");
			while($upr = <UPR>){
				$upr =~ s/\+FileName\+/$typedef_fpp_func{$typedef_name}.c/;
				$upr =~ s/\+Warning\+/\$type???로 정의된 것들만 사용가능/;
				$upr =~ s/\+ToDo\+/library를 만들기 위한 Makefile을 만들자/;
				$upr =~ s/\+Intro\+/typedef를 위한 functions/;
				$upr =~ s/\+Requirement\+/규칙에 틀린 곳을 찾아주세요./;
	...
	print_fp ("$function_def\{\n",DBG,PRT);
	print_fp("\t"x1 . "FPRINTF(LOG_LEVEL,\"_%s : pthis = %p" . "\",pcPrtPrefixStr, pthis)\;\n",DBG,PRT);
	...
	foreach $member_vars (@member_vars){
		...
		if($member_vars =~ /^\s*BIT(\d+)\s+(\w+)\s*\(\s*(\d+)\s*\:\s*(\w+)\s*\)\s*$/){ # BIT인 경우
			...
			if($bit_sum >= $bit_tot){
				...
				print_fp("\t"x1 . "FPRINTF(LOG_LEVEL,\"_%s : pthis->$bit_struct\.A : $bit_tot = %p" .  "\",pcPrtPrefixStr,pthis->$bit_struct\_A" . ")\;\n",DBG,PRT);
				foreach $temp_vars (@bit_array){
					...
					if($temp_vars =~ /^\s*BIT(\d+)\s+(\w+)\s*\(\s*(\d+)\s*\:\s*(\w+)\s*\)\s*$/){
						...
						$bit_struct = $4;
						$bit_comment = $5;
						print_fp("\t"x1 . "FPRINTF(LOG_LEVEL,\"_%s : pthis->$bit_struct\.B\.$bit_name : $bit_cnt = \%d" .  "\",pcPrtPrefixStr,pthis->$bit_struct\_B\_$bit_name" . ")\;\n",DBG,PRT);
		...
		if( $member_vars =~ /^struct/){ 	 ## struct로 시작하는 것일 경우 
			...
			if($member_vars =~ /struct\s+(\S+)\s+(\S+)/){
				...
				if($member_name =~ s/^\*+//){
					...
					print_fp("\t"x1 . "FPRINTF(LOG_LEVEL,\"_%s : pthis->$member_name = $type_printV{structp}" . "\",pcPrtPrefixStr,$type_printM{structp}" . "(pthis->$member_name))\;\n",DBG,PRT);
				}
				#변수들이 정의가 다 된 것이다.
				# 이 안에서만 정의를 하면 될 것이다.  
...
#print "A1 $typedef_name , $member_type , $member_name , $table_log{$typedef_name} , $typedef{$member_type}\n";
					...
					if(not $member_array_size ){
						...
						print_fp("\t"x1 . "FPRINTF(LOG_LEVEL,\"_%s : pthis->$member_name = $type_printV{U32}" . 
								"\",pcPrtPrefixStr,$type_printM{OFFSET}" . "(pthis->$member_name))\;\n",DBG,PRT);
						...
						if(("U8" eq $member_type) || ("S8" eq $member_type) || ("STRING" eq $member_type)){
							...
							print_fp("\t"x1 . "FPRINTF(LOG_LEVEL,\"_%s : pthis->$member_array_name" . " = \%s" . "\" ,pcPrtPrefixStr,pthis->$member_array_name)\;\n",DBG,PRT);
							...
							if($typedef_prt_func{$member_type}){
								...
								print_fp("\t"x3 . "FPRINTF(LOG_LEVEL,\"_%s : pthis->$member_array_name" . "[\%4d] = $type_printV{$member_type}" . "\" ,pcPrtPrefixStr,iIndex, $type_printM{$member_type}" . "(pthis->$member_array_name)" . "[iIndex])\;\n",DBG,PRT);
							}
							print_fp("\t"x2 . "}\n",DBG,ENC,DEC,PRT);
							print_fp("\t"x1 . "}\n",DBG,ENC,DEC,PRT);
						}
...
#print "B1 $typedef_name , $member_type , $member_name , $table_log{$typedef_name} , $typedef{$member_type}\n";
					...
					if($type{$member_type}){
						...
						###	print_fp("\t\{    $type_printPre{$member_type}  $member_type   $member_name\n",DBG,PRT);
							...
							print_fp("\t\{    $printPre \n",DBG,PRT,FPP);
							print_fp("\t"x2 . "FPRINTF(LOG_LEVEL,\"_%s : pthis->$member_name = $type_printV{$member_type}" . "\",pcPrtPrefixStr,$type_printM{$member_type}" . ")\;\n",DBG,PRT);
							print_fp("\t"x2 . "FILEPRINT(fp,\"$type_printV{$member_type}" . "$file_delimiter\",$type_printM{$member_type}" . ")\;\n",DBG,FPP);
							print_fp("\t\}\n",DBG,PRT,FPP);
						} else{
							print_fp("\t"x1 . "FPRINTF(LOG_LEVEL,\"_%s : pthis->$member_name = $type_printV{$member_type}" . "\",pcPrtPrefixStr,$type_printM{$member_type}" . "(pthis->$member_name))\;\n",DBG,PRT);
...
print_fp("\t"x1 . "$member_type" . "_CILOG(fp, &(pthis->$member_name));\n" ,DBG,FPP);
	...
	if(not $typedef_name =~ /$shortprefix/){
		++$typedef_def_cnt;
		my $hex = sprintf '%p' ,$typedef_def_cnt;
...
#print DBG "\^ANALYSIS_TYPEDEF : $parms\n";
	...
	foreach $member_vars (@member_vars){
		...
		print DBG "ANALYSIS : [$member_vars]\n";
		if($member_vars =~ /\/\*\*<([^\/]+)\*\// ){
			$table_comments = $1;
		}
		$member_vars =~ s/\%.*\%//g;
		...
		print DBG "member_vars 2 : $typedef_name : TAG $analysis_tag_define : VARS $member_vars\n";
		## 앞의 \s를 없앤다. (white space)
		if($member_vars =~ /\s*(\w+)\s+([^:;]+)/){
			$member_type = $1;
			$member_name = $2;
			$member_name =~ s/\s//g;
		}
		...
		print DBG "member_vars 3 : TYPE $member_type : NAME $member_name\n";
		if($member_name =~ s/\*//){
			$member_pointer = "YES";
		}
		...
		print DBG "member_vars 4 : TYPE $member_type : NAME $member_name : ARRAY $member_array : SIZE $member_array_size\n";
		...
		if($type{$member_type}){
			...
			$undefined_typedef{$undefined_name} = "HASH_$undefined_name";
			$$undefined_name{$member_name} = "\/** $member_vars  : HIDDEN = " . "$cilog_hidden *\/";
...
# ??? LIST를 Gourping하여 그 값을 대표적으로 사용하게 하는 것을 마드러야  할 것이다. 
			...
			$undefined_name = "ANALYSIS_$typedef_name\_COMMENTS";
			$undefined_typedef{$undefined_name} = "HASH_$undefined_name";
			$$undefined_name{$member_name} = "\/** $member_vars *\/";
		} else {
			print DBG "()()() : unknown $member_vars\n";
		}
...
sub flat_save_typedef
{
	...
	$typedef_enc_func{$typedef_name} = $typedef_name . $primitives{"ENC"};
	open(ProC , ">$outputdir/$typedef{$typedef_name}" . ".pc");
	$filelist{"$typedef{$typedef_name}" . ".pc"} = "ProC";
			open(UPR,"<header.upr");
			while($upr = <UPR>){
				$upr =~ s/\+FileName\+/$typedef{$typedef_name}.pc/;
				$upr =~ s/\+Warning\+/NTAM용으로 만드는 앞에 flat_st 가 모두 들어갈 것이다./;
				$upr =~ s/\+ToDo\+//;
				$upr =~ s/\+Intro\+/DB함수 : $typedef{$typedef_name}을 DB에 넣는 함수/;
				$upr =~ s/\+Requirement\+/DB Table의 이름이 $typedef{$typedef_name} 과 같아야 한다. /;
				print ProC $upr;
			}
			close UPR;
...
sub stg_hash_key2
{
	...
	open(COMBI , ">$outputdir/$set_combi_func" . "\.c");
	$filelist{$set_combi_func . "\.c"} = "CFILE";
			open(UPR,"<header.upr");
			while($upr = <UPR>){
	...
	open(ACCUM , ">$outputdir/$set_accumulate_func" . "\.c");
		$filelist{$set_accumulate_func . "\.c"} = "CFILE";
			open(UPR,"<header.upr");
			while($upr = <UPR>){
...
sub stat_function {
	...
	open(STATACCUM , ">$outputdir/$set_stat_file" . "\.c");
	$filelist{$set_stat_file . "\.c"} = "CFILE";
			open(UPR,"<header.upr");
			while($upr = <UPR>){
...
#print_fp("\n",DBG,STATACCUM);
	...
	open(STATONCE , ">$outputdir/$set_stat_file" . "\.c");
	$filelist{$set_stat_file . "\.c"} = "CFILE";
			open(UPR,"<header.upr");
			while($upr = <UPR>){
				$upr =~ s/\+FileName\+/$set_stat_file\.c/;
...
while ($line = <STG>) {
	$analysis_line = $line;
	$line =~ s/\@[^@]*\@//;		## CILOG_HIDDEN , CHECKING_VALUE등을 없애줌. 단지 analysis_line에서만 처리해주게 됨.
	$case_ignore = "NO";
...
## 		 다음문자의 숫자가 0 이면 끝이므로 return값을 넣어줌 
	...
	if (($tag_auto_define_name ne "") && ($line =~ /^\s*(\w+)\s*(.*)$/)){
		my $tab_cnt =  int((47 - length($1)) /4);
		$temp_argu1 = $1;
		$temp_argu2 = $2;
		...
		if($line =~ s/#(\S*)#//g){
			...
			$temp_argu2 =~ s/#(\S*)#/\/* abbreviation_name:$1 *\/\t/;
		} else {
			$abbreviation_define_name{$temp_argu1} = $temp_argu1;
		}
		$line = "\#define $temp_argu1" . "\t"x$tab_cnt . "$tag_auto_define_start_num		$temp_argu2";
		print DBG "$line\n";
		if($in_typedef == 1) {
			$typedef_org .= $line . "\n";
		} else {
	...
	if ($line =~ /^\s*\<\s*TAG_FLOW_START\s*\:\s*(\w+)\s*\(\s*(.+)\s*\)\s*\>/){ 
		print DBG "TAG_FLOW $line : name = $1\n";
		$tag_flow_name = $1;
		$temp = $2;
...
#print "var : $if_var\n";
		...
		if($line =~ /#{(.*)$/){
			$action = $1;
			$sharp_start = 1; 
			print DBG "action start = $action\n";
		}

		print DBG "\FLOW \[$1\] \[$2\] $3 $4 $5 $6 $7 \#$8\#\n";
		next;
	}
	if( ($tag_flow_name ne "") && ($sharp_start == 1) ){
		if($line =~ /(.*)\}#\s*$/){
			$action .= "\n$1";
			flow_undefined();
			print DBG "action_end = $action\n";
			$sharp_start = 0;
	...
	if($in_typedef == 1) {
		if($line =~ /^\s*LINEFEED\s+(.*)$/){
			...
			print DBG "LINEFEED = " . $text_parse_linefeed . "\n";
			next;
		}
		if($line =~ /^\s*FORMFEED\s+(.*)$/){
			$text_parse_formfeed = $1;
			print DBG "FORMFEED = " . $text_parse_formfeed . "\n";
			next;
		}
		if($line =~ /^\s*STATE\s+(.*)$/){
			$text_parse_state .= $1 . "#^#^#^#";
			print DBG "STATE = " . $text_parse_state . "\n";
			next;
		}
		if($line =~ /^\s*TOKEN\s+(.*)$/){
			$text_parse_token .= $1 . "#^#^#^#";
			print DBG "TOKEN = " . $text_parse_token . "\n";
			next;
		}
		if($line =~ s/^\s*ALTERNATIVE_RULE\s+(.*)$/$1/){
			print DBG  "TK ALTERNATIVE_RULE = " . $line . "\n";
			$prev_line =~ s/^(.*;).*$/$1/;
			print  DBG "ALTERNATIVE : [[$prev_line]] : $line\n";
			$line =~ s/\/\*.+\*\///g;
...
print DBG "A1 $tag_key_lines\n";
		...
		if($line =~ /^\s*(\w+)\s+(\w+)\s*\[\s*(\d+)\s*\]\s*\;/){	##  [16] array로 선언될때의 처리
			...
			print DBG "=$1 =$2 =$3 \+$define_digit{$3}\n";
			if($short eq ""){ $short = $2; }
			if($typedef{$1}){
...
#print "$line\n";
			...
			if($bit_sum >= $bit_tot){
				...
				foreach $temp_vars (@bit_array){
					...
					if($temp_vars =~ /^\s*BIT(\d+)\s+(\w+)\s*\(\s*(\d+)\s*\:\s*(\w+)\s*\)\s*\;\s*(.*)$/){
						...
						print DBG "DBG_TYPEDEF BIT($line) ==> \n$typedef\n";
						print DBG "DBG_TYPEDEF_CR BIT($line) ==> \n$typedef_cr\n";
						print DBG "DBG_TYPEDEF_ORG BIT($line_prt) ==> \n$typedef_org\n";
						print DBG "DBG_TYPEDEF_SHORT BIT($line_short) ==> \n$typedef_short\n";
					}
				}
		...
		print DBG "DBG_TYPEDEF ($line) ==> \n$typedef\n";
		print DBG "DBG_TYPEDEF_CR ($line) ==> \n$typedef_cr\n";
		print DBG "DBG_TYPEDEF_ORG ($line_prt) ==> \n$typedef_org\n";
		print DBG "DBG_TYPEDEF_SHORT ($line_short) ==> \n$typedef_short\n";
		print DBG "short \$line_short ==> \n$line_short";
		$line_short = "";
		### /* ... */ 주석을 삭제
	...
	if($in_typedef == 2) {
		...
		$typedef_short =~ s/\n//g;
		flat_save_typedef($typedef_short); 
...
#print DBG "Process_typedef = $typedef\n";
		...
		if($typedef =~ /^\s*STG_LOG_TEXT_PARSING\s+typedef\s+struct/){ 
			my $from;
...
#print DBG "TTTT_TTTT DBG_TYPEDEF_CR ==> $typedef_cr\n";
			...
			foreach $temp_vars (@temp_vars){
				...
				if($temp_vars =~ s/^\s*ALTERNATIVE_RULE\s+(.*)$/$1/){
					...
					print  DBG "FOR_ALTERNATIVE : $temp_vars\n";
				...
				if($temp_vars =~ /^\s*(\S+)\s+(\S+)\;\s*\#(.*)\#\s*$/){
					...
					print DBG "TT size = $var_array_size\n";
					...
					if( ($cmd_n_rule =~ /PARSING_RULE\:\^\^\:(\S*)\:\^\^\:([^:]+)\:\^\^\:(\S*)$/)  or
						...
						my $temp_state;			# HOST
						print DBG "CMD_N_RULE : $cmd_n_rule ==> $temp_action\n";
...
# 편리하게 쓸수 있게 만들어주는 것이 좋지 않을까 하는 생각이 든다.
		...
		if($typedef =~ /^\s*TABLE_LOG\s+typedef\s+struct/){ 
		...
		if($typedef =~ /^\s*TABLE_CF\s+typedef\s+struct/){ 
	...
	print_all_global_var("GLOBAL_Pre_STC.TXT");

	print "\n\n\n======== STC START =========\nstc_filearray : @stc_filearray\n";
	foreach my $stcfile (@stc_filearray){
		if($stcfile =~ /\.stc/){
										# 3번째 인자 : stcL이 아닌 경우는 NULL
			Cchange($stcfile,"STC","");			# OUTPUT안에 저장하고, makefile에도 넣어라.
		}
		if($stcfile =~ /\.stcI/){
	...
	while($in = <UPR>){
		...
		if($in =~ /\.stcI/){
			Ichange($in,"");			# OUTPUT안에 저장하고, makefile에도 넣어라.
										...
										# 3번째 인자 : stcL이 아닌 경우는 NULL
			Cchange($in,"","");			# OUTPUT안에 저장하고, makefile에도 넣어라.
		}
	}
	close UPR;
	print_all_global_var("GLOBAL.TXT");
		...
		if((keys %stat_typedef)){	# 한개도 STAT가 없으면 이것을 처리하지 않는다.
			...
			foreach $def (keys %typedef_def_cnt){
				my $hex = sprintf '%p' ,$typedef_def_cnt{$def};
...
$ret = 1;
		if(1 eq $ret){
			...
			open(SQL,">$outputdir/$comp" . "\.sql");
			if($comp =~ /^\s*STAT\s*/){
				print_fp("create table " . $comp . "\n\(\n" , SQL,DBG);
			}
			else {
				print_fp("create table " . $comp . "_\%s\n\(\n" , SQL,DBG);
			}
			...
			foreach my $member_vars (@member_vars){
				if($member_vars =~ /\s*(\w+)\s+(\w+)\s*\[(\d*)\]/){	##  [] array로 선언될때의 처리
					...
					$varname =~ s/$shortremoveprefix//g;
					print_fp("\t$varname		$type_CREATE{$vartype} 	\,\n",SQL,DBG);
				}
			}
...
##
## Revision 1.1  2011/08/03 06:02:43  uamyd
## CGA, HASHO, TIMERN library added
##
## Revision 1.5  2011/05/20 09:50:09  jsyoon
## *** empty log message ***
##
## Revision 1.4  2011/01/11 04:09:04  uamyd
## modified
##
## Revision 1.1.1.1  2010/08/23 01:13:06  uamyd
## DQMS With TOTMON, 2nd-import
##
## Revision 1.3  2010/03/08 12:31:28  dqms
## *** empty log message ***
##
## Revision 1.2  2009/06/29 08:30:07  dqms
## *** empty log message ***
##
## Revision 1.1  2009/06/10 16:45:50  dqms
## *** empty log message ***
##
## Revision 1.1.1.1  2009/05/26 02:13:18  dqms
## Init TAF_RPPI
##
## Revision 1.3  2008/11/17 11:09:57  dark264sh
## *** empty log message ***
##
...
##
## Revision 1.3  2007/09/05 11:00:14  dark264sh
## *** empty log message ***
##
## Revision 1.2  2007/08/31 10:55:41  dark264sh
## *** empty log message ***
##

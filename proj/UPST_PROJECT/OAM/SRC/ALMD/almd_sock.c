...
int dMakeSwitchMngPkt(st_MngPkt *stPkt, st_SWITCH_MNG *pstSWITCH)
{
	/* DIRECTOR STATUS */
	stPkt->head.llMagicNumber	= MAGIC_NUMBER;
	stPkt->head.usTotLen		= MNG_PKT_HEAD_SIZE + sizeof(st_SWITCH_MNG);
	stPkt->head.usBodyLen		= sizeof(st_SWITCH_MNG);
	stPkt->head.ucNTAMID		= 0;
	stPkt->head.ucNTAFID		= 0;
	stPkt->head.ucSysNo			= 0;
	stPkt->head.ucSvcID			= SID_STATUS_SWITCH;
	stPkt->head.ucMsgID			= MID_ALARM;

	memcpy(stPkt->data, pstSWITCH, sizeof(st_SWITCH_MNG));

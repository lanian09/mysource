...
int dMsgrcv(pst_MsgQ *ppstMsg)
{
	...
	if( (gdOffset = gifo_read(gpMEMSINFO, gpCIFO, SEQ_PROC_CI_SVC)) <= 0 ){
		usleep(0);
		return -1;
	}
...
int dMsgsnd(int procID, U8 *pNODE)
{
	...
	if(gifo_write(gpMEMSINFO, gpCIFO, SEQ_PROC_CI_SVC, procID, offset) < 0) {
        log_print(LOGN_CRI, LH"FAILED IN gifo_write(CI_SVC:%d > TARGET:%d), offset=%ld"EH,
                LT, SEQ_PROC_CI_SVC, procID, offset, ET);
		nifo_node_delete(gpMEMSINFO, pNODE);
        usleep(0);
        return -1;
    }
    log_print(LOGN_INFO,LH"SND CI_SVC:%d TARGET:%d, offset=%ld",
            LT, SEQ_PROC_CI_SVC, procID, offset);
    return 0;
}
...
int dMsgsnd2(int procID)
{
	...
	if(gifo_write(gpMEMSINFO, gpCIFO, SEQ_PROC_CI_SVC, procID, gdOffset) < 0) {
        log_print(LOGN_CRI, LH"FAILED IN gifo_write(CI_SVC:%d > TARGET:%d), gdOffset=%ld"EH,
                LT, SEQ_PROC_CI_SVC, procID, gdOffset, ET);
		nifo_node_delete(gpMEMSINFO, nifo_ptr(gpMEMSINFO, gdOffset));
        usleep(0);
        return -1;
    }

    log_print(LOGN_INFO,LH"SND CI_SVC:%d TARGET:%d, gdOffset=%ld",
            LT, SEQ_PROC_CI_SVC, procID, gdOffset);
	return 0;
}

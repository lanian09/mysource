...
/******* Modified By KIW in 2004.2.17 to LGT Project ****/
typedef	struct	_TCP_RHD_T {
	UCHAR	Source[2];
	UCHAR	Destination[2];
	UCHAR	Seq[4];
	UCHAR	Ack[4];
	...
	UCHAR	UrgPoint[2];
	UCHAR	Reserved[4];
} TCP_RHD_T, *PTCP_RHD_T;


#if 0
/*** dark264 20040315 Block ***/
/* TCP Header, RFC793 */
...
typedef struct _TCP_RHDR {
	...
	union {
		...
		struct {
			...
			UCHAR	CB_ACK:1;
			UCHAR	CB_URG:1;
			UCHAR	ECN_ECE:1;		/* RFC 3168 */
			UCHAR	ECN_CWR:1;		/* RFC 3168 */
			};
		};
	UCHAR	Window[2];
	UCHAR	Checksum[2];

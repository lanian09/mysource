 ...
 *
 *		$Id: memg.c,v 1.2 2011/08/31 04:26:04 uamyd Exp $
 *
...
memg_init(U32 uiType,U32 uiShmKey,U32 uiHeadRoomSize, U32 uiMemNodeBodySize,U32 uiMemNodeTotCnt)
{
	...
	stMEMGINFO *pstMEMGINFO = NULL;
	size_t	uiTotMemSize;
	int		shm_id;

	switch(uiType){
		case MEMG_MAIN_MEM:
    		uiTotMemSize = (size_t) (stMEMGINFO_SIZE + uiHeadRoomSize 
		...
		case MEMG_SHARED_MEM:
            ...
            if ((shm_id = shmget (uiShmKey, uiTotMemSize, SEMPERM | IPC_CREAT | IPC_EXCL)) < 0) {
				...
				if (errno == EEXIST) {
                	...
                	if (shm_id < 0) {
						FPRINTF(LOG_LEVEL,"##%s Shm create fail!!: exit()",__FUNCTION__);
						return NULL;
					}
...
#endif
                	if((pstMEMGINFO = (stMEMGINFO *) shmat(shm_id, 0, 0)) == (void *)-1)
					{
	...
	pHeadRoom = (U8 *)(pBase + pstMEMGINFO->offsetHeadRoom);
	memset(pHeadRoom, 0x00, pstMEMGINFO->uiHeadRoomSize);
	...
	for(iIndex=0; iIndex < pstMEMGINFO->uiMemNodeTotCnt; iIndex++) {
		...
		pstMEMGNODEHDR->TimeSec = 0;
		pstMEMGNODEHDR->ucIsFree = MEMG_FREE;
		pstMEMGNODEHDR->DebugStr[0] = 0x00;
#ifdef DEBUG
...
/** 초기화 함수 : memg_reset function. 
 *
 *  초기화 함수 
 *
 * @param *pstMEMSINFO	: stMEMGINFO
 *
 *  @return     void
 *  @see        memg.h
 *
 ...
 **/
void 
memg_reset(stMEMGINFO *pstMEMGINFO, size_t base)
{
	U32	iIndex;
	stMEMGNODEHDR *pstMEMGNODEHDR;
	U8 *pBase,*pIndex, *pHeadRoom;
	...
	pBase = (U8 *)(pstMEMGINFO); 
	pHeadRoom = (U8 *)(pBase + pstMEMGINFO->offsetHeadRoom + base);
	memset(pHeadRoom, 0x00, pstMEMGINFO->uiHeadRoomSize - base);
	pBase += pstMEMGINFO->offsetNodeStart;
	for(iIndex=0; iIndex < pstMEMGINFO->uiMemNodeTotCnt; iIndex++) {
		pIndex = (U8*)(pBase + iIndex * (stMEMGNODEHDR_SIZE + pstMEMGINFO->uiMemNodeBodySize)); 
		pstMEMGNODEHDR = (stMEMGNODEHDR *)pIndex;
...
memg_print_all(S8 *pcPrtPrefixStr,stMEMGINFO *pstMEMGINFO)
{
	...
	FPRINTF(LOG_LEVEL,"%s ##1: pstMEMGINFO=%p \n",__FUNCTION__, pstMEMGINFO);
	...
	for(iIndex=0;iIndex < pstMEMGINFO->uiMemNodeTotCnt;iIndex++){
		pIndex = (U8*) (pBase + iIndex * (stMEMGNODEHDR_SIZE + pstMEMGINFO->uiMemNodeBodySize)); 
		pstMEMGNODEHDR = (stMEMGNODEHDR *) pIndex;
		FPRINTF(LOG_LEVEL,"%s ##2: %s : [%3d] ID :0x%x pIndex=%p\n",pcPrtPrefixStr,__FUNCTION__,iIndex,pstMEMGNODEHDR->uiID, pIndex);
		FPRINTF(LOG_LEVEL,"%s ##2: offset %ld\n",__FUNCTION__,iIndex * (stMEMGNODEHDR_SIZE + pstMEMGINFO->uiMemNodeBodySize));
...
memg_draw_all(S8 *filename,S8 *labelname,stMEMGINFO *pstMEMGINFO)
{
	...
	for(iIndex=0;iIndex < pstMEMGINFO->uiMemNodeTotCnt;iIndex++){
		...
		if(iIndex == pstMEMGINFO->uiMemNodeTotCnt -1){
			FILEPRINT(fp,"<f%d> %d_%d_%s",iIndex,iIndex,pstMEMGNODEHDR->ucIsFree,pstMEMGNODEHDR->DebugStr);
		} else {
			FILEPRINT(fp,"<f%d> %d_%d_%s |",iIndex,iIndex,pstMEMGNODEHDR->ucIsFree,pstMEMGNODEHDR->DebugStr);
		}
	}
...
/** file memg.c
 *     $Log: memg.c,v $
 *     Revision 1.2  2011/08/31 04:26:04  uamyd
 *     modified
 *
 ...
 *
 *     Revision 1.5  2011/01/11 04:09:03  uamyd
 *     modified
 *
 *     Revision 1.1.1.1  2010/08/23 01:13:07  uamyd
 *     DQMS With TOTMON, 2nd-import
 *
 *     Revision 1.4  2009/06/28 14:52:00  dark264sh
 *     *** empty log message ***
 *
 *     Revision 1.3  2009/06/28 14:37:46  dark264sh
 *     *** empty log message ***
 *
 *     Revision 1.2  2009/06/18 07:26:00  dark264sh
 *     add hasho_reset function
 *
 *     Revision 1.1  2009/06/10 16:45:50  dqms
 *     *** empty log message ***
 *
 *     Revision 1.1.1.1  2009/05/26 02:13:42  dqms
 *     Init TAF_RPPI
 *
 *     Revision 1.3  2008/11/17 11:07:07  dark264sh
 *     *** empty log message ***
 *
 *     Revision 1.2  2008/11/17 08:56:53  dark264sh
 *     64bits 작업
 *
 *     Revision 1.1.1.1  2008/06/09 08:17:20  jsyoon
 *     WATAS3 PROJECT START
 *
 *     Revision 1.2  2007/08/29 07:43:15  dark264sh
 *     *** empty log message ***
 *
 *     Revision 1.1  2007/08/21 12:19:12  dark264sh
 *     no message
 *

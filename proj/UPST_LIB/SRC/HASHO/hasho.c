 ...
 * Hash Library file.
 *
 *		$Id: hasho.c,v 1.1.1.1 2011/08/19 00:53:28 uamyd Exp $
 *
...
OFFSET (*hasho_func)(void *pa, U8 *pb);
 ...
 **/
OFFSET hasho_func_default(void *pa,U8 *pb)
{
 ...
 **/
stHASHOINFO *
hasho_init(U32 uiShmKey, U16 usKeyLen, U16 usSortKeyLen, U16 usDataLen, U32 uiHashSize, OFFSET (*func)(void*,U8*))
{
	...
	if (uiShmKey == 0) {
		pstMEMGINFO = memg_init(MEMG_MAIN_MEM
					...
					,uiHashSize * 5);  /* 항시 10이 아닐수 있음. 변화 가능 */
		if(pstMEMGINFO == NULL) return NULL;
	} else {
	...
	pstHASHOINFO->offset_memginfo	= (OFFSET) HASHO_OFFSET(pstHASHOINFO, pstMEMGINFO);
#ifdef DEBUG
...
/** 기본 hash function : hasho_reset function. 
 *
 * HASH를 초기화 한다. 
 * 
 *
 ...
 **/
void 
hasho_reset(stHASHOINFO *pstHASHOINFO)
{
	memg_reset((stMEMGINFO *)HASHO_PTR(pstHASHOINFO, pstHASHOINFO->offset_memginfo), stHASHOINFO_SIZE);
}
...
hasho_print_node(S8 *pcPrtPrefixStr,stHASHOINFO *pstHASHOINFO,stHASHONODE *pstHASHONODE)
{
	...
	hasho_dump_DebugString("KEY",(char*)HASHO_PTR(pstHASHOINFO, pstHASHONODE->offset_Key),pstHASHOINFO->usKeyLen);
	hasho_dump_DebugString("DATA",(char*)HASHO_PTR(pstHASHOINFO, pstHASHONODE->offset_Data),pstHASHOINFO->usDataLen);
}
...
hasho_print_all(S8 *pcPrtPrefixStr,stHASHOINFO *pstHASHOINFO)
{
	...
	FPRINTF(LOG_LEVEL,"%s : pstHASHOINFO=%p , p->psthashnode=%p\n",__FUNCTION__, pstHASHOINFO, HASHO_PTR(pstHASHOINFO, pstHASHOINFO->offset_psthashnode));
	...
	for(iIndex=0;iIndex < pstHASHOINFO->uiHashSize;iIndex++){
		...
		if(*pNodeHead){
			FPRINTF(LOG_LEVEL,"##%s : %s : [%3d] location : %p , valuep=%ld\n"
					,pcPrtPrefixStr
					,__FUNCTION__,iIndex,
					(((OFFSET *) HASHO_PTR(pstHASHOINFO, pstHASHOINFO->offset_psthashnode)) + iIndex),
					*(((OFFSET *) HASHO_PTR(pstHASHOINFO, pstHASHOINFO->offset_psthashnode)) + iIndex));
			pOff = (OFFSET) *pNodeHead;
			while(pOff){
				p = (stHASHONODE *) HASHO_PTR(pstHASHOINFO, pOff);
				FPRINTF(LOG_LEVEL,"##%s : [%3d] p=%p , p offset=%ld\n",__FUNCTION__,iIndex, p, HASHO_OFFSET(pstHASHOINFO, p));
				stHASHONODE_Prt(pcPrtPrefixStr,p);
				hasho_dump_DebugString("## : KEY",(char *) HASHO_PTR(pstHASHOINFO, p->offset_Key),pstHASHOINFO->usKeyLen);
...
hasho_draw_all(S8 *filename,S8 *labelname,stHASHOINFO *pstHASHOINFO)
{
	...
	fp = fopen(filename,"w");
	FILEPRINT(fp,"/** @file %s\n",filename);
	FILEPRINT(fp,"pstHASHOINFO->uiHashSize = %d \\n\n",pstHASHOINFO->uiHashSize);
	FILEPRINT(fp,"pstMEMGINFO->uiMemNodeTotCnt = %d \\n\n",pstMEMGINFO->uiMemNodeTotCnt);
	FILEPRINT(fp,"pstMEMGINFO->uiMemNodeAllocedCnt = %d \\n\n",pstMEMGINFO->uiMemNodeAllocedCnt);
	FILEPRINT(fp,"\\dot \n	\
	digraph G{ 	\n\
	fontname=Helvetica; 	\n\
	label=\"Hash Table(%s)\"; 	\n\
	...
	for(iIndex=0;iIndex < pstHASHOINFO->uiHashSize;iIndex++){
		if(iIndex == (pstHASHOINFO->uiHashSize -1)){
			FILEPRINT(fp,"<f%d> %d ",iIndex,iIndex);
		} else {
			FILEPRINT(fp,"<f%d> %d| ",iIndex,iIndex);
		}
	}
	...
	for(iIndex=0;iIndex < pstHASHOINFO->uiHashSize;iIndex++){
		...
		while(pOff){
			p = (stHASHONODE *) HASHO_PTR(pstHASHOINFO, pOff);
			FILEPRINT(fp,"N%p [label = \"{ <n> %p | %3d\\ %ld | <p> }\"];\n", p,  p,iIndex,pOff);
			pOff = p->offset_next;
		}
	}
	FILEPRINT(fp,"\n\n");
	for(iIndex=0;iIndex < pstHASHOINFO->uiHashSize;iIndex++){
		...
		if(pOff){
			p = (stHASHONODE *) HASHO_PTR(pstHASHOINFO, pOff);
			FILEPRINT(fp,"node0:f%d -> N%p:n;\n",iIndex, p);
		}
		while(pOff){
			p = (stHASHONODE *) HASHO_PTR(pstHASHOINFO, pOff);
			pOff = p->offset_next;
			if(pOff){
			   	FILEPRINT(fp,"N%p:p -> N%p:n;\n", p , HASHO_PTR(pstHASHOINFO, p->offset_next));
			}
		}
	}
	FILEPRINT(fp,"}\n\\enddot \n\n");
 ...
 *     upst library
 *
 *     Revision 1.1  2011/08/03 06:02:46  uamyd
 *     CGA, HASHO, TIMERN library added
 *
 *     Revision 1.6  2011/01/11 04:09:03  uamyd
 *     modified
 *
 *     Revision 1.1.1.1  2010/08/23 01:13:07  uamyd
 *     DQMS With TOTMON, 2nd-import
 *
 *     Revision 1.5  2010/03/18 11:27:49  dqms
 *     hasho hasho_init 실패시 NULL값 리턴
 *
 *     Revision 1.4  2009/09/22 15:27:34  pkg
 *     *** empty log message ***
 *
 *     Revision 1.3  2009/06/28 14:37:21  dark264sh
 *     *** empty log message ***
 *
 *     Revision 1.2  2009/06/18 07:25:20  dark264sh
 *     add hasho_reset function
 *
 *     Revision 1.1  2009/06/10 16:45:50  dqms
 *     *** empty log message ***
 *
 *     Revision 1.1.1.1  2009/05/26 02:13:38  dqms
 *     Init TAF_RPPI
 *
 *     Revision 1.3  2008/11/17 11:06:16  dark264sh
 *     *** empty log message ***
 *
 *     Revision 1.2  2008/11/17 08:56:06  dark264sh
 *     64bits 작업
 *
 *     Revision 1.1.1.1  2008/06/09 08:17:20  jsyoon
 *     WATAS3 PROJECT START
 *
 *     Revision 1.2  2007/08/29 07:43:29  dark264sh
 *     *** empty log message ***
 *
 *     Revision 1.1  2007/08/21 12:18:32  dark264sh
 *     no message
 *

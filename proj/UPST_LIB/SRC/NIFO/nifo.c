 ...
 *
 *	$Id: nifo.c,v 1.3 2011/08/31 04:26:05 uamyd Exp $
 *
 ...
 *			@li	nifo_splice_nont, nifo_splice_cont
 *			@li	nifo_tlv_alloc
 *			@li nifo_msg_write
 *			@li nifo_msg_read
...
OFFSET nifo_get_offset_node(stMEMSINFO *pMEMSINFO, U8 *ptr)
{
	...
	OFFSET 			cnt;
	S32				i;
	st_MEMSZONE		*pMEMSZONE;

	base = nifo_offset(pMEMSINFO, ptr);

	for(i = 0; i < pMEMSINFO->uiZoneCnt; i++)
	{
		pMEMSZONE = &pMEMSINFO->stMEMSZONE[i];

		if(pMEMSZONE->offsetNodeStart <= base && pMEMSZONE->offsetNodeEnd > base)
		{
			break;
		}
	}

	if(i == pMEMSINFO->uiZoneCnt) return 0;

	base = base - pMEMSZONE->offsetNodeStart;	

	cnt = base / (pMEMSINFO->uiMemNodeHdrSize + pMEMSZONE->uiMemNodeBodySize);

	base = pMEMSZONE->offsetNodeStart + (pMEMSINFO->uiMemNodeHdrSize + pMEMSZONE->uiMemNodeBodySize) * cnt + pMEMSINFO->uiMemNodeHdrSize;
    ...
    nifo_node_for_each_start(pMEMSINFO, pNont, &pTmp->nont) {
        ...
        nifo_node_for_each_start(pMEMSINFO, pCont, &pNode->cont) {
			...
			if((MEMS_ID != pMEMSNODEHDR->uiID) || (MEMS_ALLOCED != pMEMSNODEHDR->ucIsFree)) {
				FPRINTF(LOG_BUG, "%s() ALLOC PROCESS[%s] FREE PROCESS[%s] ID[%u][%u]TIME[%u]ISFREE[%d] NONTCNT[%d] CONTCNT[%d]\n", __FUNCTION__,
 ...
 *  @note       Nothing
 **/
stMEMSINFO *nifo_init(st_MEMSCONF *pMEMSCONF, U8 *pDbgStr, S32 processID)
{
...
//	return mems_init(MEMS_SHARED_MEM, uiShmKey, MEMS_SEMA_ON, uiSemKey, DEF_HEADROOM_SIZE, DEF_MEMNODEBODY_SIZE, DEF_MEMNODETOT_CNT);
	return mems_init(pMEMSCONF);
}

/** nifo_init_zone function. 
 *
 *  초기화 함수 
 *
 * 	@param *pDbgStr: Process Name (For Debugging)
 * 	@param processID: Process ID (For Debugging)
 * 	@param *confFile: config File Name ("/DIFO/DATA/nifo_zone.conf")
 *
 *  @return     stMEMSINFO *	(MEM 관리자 Pointer)
 *  @see        nifo.h nifo.c
 *
...
stMEMSINFO *nifo_init_zone(U8 *pDbgStr, S32 processID, S8 *confFile)
{
	S32				dRet;
	st_MEMSCONF		stMEMSCONF;
	st_MEMSCONF		*pMEMSCONF = &stMEMSCONF;

	if((dRet = nifo_zone_conf_init(pMEMSCONF, confFile)) < 0)
	{
		FPRINTF(LOG_BUG, "F=%s:%s.%d nifo_zone_conf_init dRet=%d\n", __FILE__, __FUNCTION__, __LINE__, dRet);
		return NULL;
	}

	/* config print */
#ifdef DEBUG
	nifo_conf_print(pMEMSCONF);
#endif
	
	return nifo_conf_check(pMEMSCONF) < 0 ? NULL : nifo_init(pMEMSCONF, pDbgStr, processID);
}

S32 nifo_conf_check(st_MEMSCONF *pMEMSCONF)
{
	S32				i, j, dRet = 0;
	U32				zoneID;
	S32				procCnt[MAX_MEMSZONE_CNT];

	for(i = 0; i < MAX_MEMSZONE_CNT; i++)
	{
		procCnt[i] = 0;
	}
	...
	{
		zoneID = pMEMSCONF->uiMatrixZoneID[i];
	...
	for(i = 0; i < MAX_MEMSZONE_CNT; i++)
	{
		if((procCnt[i] > 1) && (pMEMSCONF->stMEMSZONECONF[i].uiSemFlag != MEMS_SEMA_ON))
		{
			FPRINTF(LOG_BUG, "HAVE TO USE SEMAPHORE ZONEID=%d PROCESS_CNT=%d\n", i, procCnt[i]);
			for(j = 0; j < MAX_SEQ_PROC_NUM; j++)
			{
				if(pMEMSCONF->uiMatrixZoneID[j] == i)
				{
					FPRINTF(LOG_BUG, "===> USE ZONEID=%d PROCESS SEQ=%d\n", i, j);
				}
			}
			dRet = -1;
		}
...
void nifo_conf_print(st_MEMSCONF *pMEMSCONF)
{
	S32 i;
	st_MEMSZONECONF *pZONECONF;

	if(pMEMSCONF == NULL)
	{
		FPRINTF(LOG_BUG, "NIFO CONFIG is NULL\n");
		return;
	}

	FPRINTF(LOG_BUG, "NIFO CONF: Type[%d], Sema key[%d], HeadRoomSize[%d], ZONE Cnt[%d]\n", 
			pMEMSCONF->uiType, pMEMSCONF->uiShmKey, pMEMSCONF->uiHeadRoomSize, pMEMSCONF->uiZoneCnt);

	for(i=0;i<pMEMSCONF->uiZoneCnt;i++)
	{
		pZONECONF = &pMEMSCONF->stMEMSZONECONF[i];
		FPRINTF(LOG_BUG, "ZONE ID[%d], Sema Flag[%d], Sema Key[%d], Node Body Size[%d], Node Total Cnt[%d]\n", 
				pZONECONF->uiZoneID, pZONECONF->uiSemFlag, pZONECONF->uiSemKey, pZONECONF->uiMemNodeBodySize, pZONECONF->uiMemNodeTotCnt);
	}

	FPRINTF(LOG_BUG, "ZONE & PROCESS MAPPING\n");

	for(i=0;i<MAX_SEQ_PROC_NUM;i++)
	{
		if(pMEMSCONF->uiMatrixZoneID[i] != INVALID_ID)
		{
			FPRINTF(LOG_BUG, "PROCESS SEQ[%d] ZONE ID[%d]\n", i, pMEMSCONF->uiMatrixZoneID[i]);
		}
	}
	
	FPRINTF(LOG_BUG, "NIFO CONF END .....\n");
}
...
U8 *nifo_node_alloc(stMEMSINFO *pstMEMSINFO)
{
	...
	uiZoneID = pstMEMSINFO->uiMatrixZoneID[procID];

	if(uiZoneID != INVALID_ID)
	{
		if((p = mems_alloc(pstMEMSINFO, pstMEMSINFO->stMEMSZONE[uiZoneID].uiMemNodeBodySize, uiZoneID, procName)) != NULL) {
			nifo_node_head_init(pstMEMSINFO, &(((NIFO *)p)->nont));
			nifo_node_head_init(pstMEMSINFO, &(((NIFO *)p)->cont));
			((NIFO *)p)->from = 0;
			((NIFO *)p)->to = 0;
			((NIFO *)p)->cnt = 0;
			((NIFO *)p)->maxoffset = pstMEMSINFO->stMEMSZONE[uiZoneID].uiMemNodeBodySize;
	...
	{
		FPRINTF(LOG_BUG, "F=%s:%s.%d NOT EXIST ZONE PROCESS SEQ=%d\n", __FILE__, __FUNCTION__, __LINE__, procID);
	}
...
S32 nifo_get_point_cont(stMEMSINFO *pstMEMSINFO, READ_VAL_LIST *pREADVALLIST, OFFSET offset)
{
	...
	for(i = 0; i < pSNode->cnt; i++) {
		U8 szBuf[8];
		if(dOffset + TLV_SIZE > pstMEMSINFO->uiMemNodeBodySize) {
			pBuf = szBuf;
			dOldLen = 0;
			dLen = TLV_SIZE;
			while(dOldLen < dLen) {
				if((dOffset + dLen - dOldLen) > pstMEMSINFO->uiMemNodeBodySize) {
					dCopySize = pstMEMSINFO->uiMemNodeBodySize - dOffset;
					memcpy(pBuf+dOldLen, pSetNode+dOffset, dCopySize);
					pSNode = clist_entry(nifo_ptr(pstMEMSINFO, (pSNode->cont).offset_next), NIFO, cont); 
					pSetNode = (U8 *)pSNode;
					dOldLen += dCopySize;
					dOffset = (dOffset + dCopySize + NIFO_SIZE) % pstMEMSINFO->uiMemNodeBodySize;
				} else {
		...
		if(dOffset + dLen > pstMEMSINFO->uiMemNodeBodySize) {
...
S32 nifo_read_tlv_cont(stMEMSINFO *pMEMSINFO, U8 *pHEAD, U32 *type, U32 *len, U8 **value, S32 *ismalloc, U8 **nexttlv)
{
	...
	pMEMSNODEHDR = (stMEMSNODEHDR *)(pHEAD - stMEMSNODEHDR_SIZE);
	pMEMSZONE = &pMEMSINFO->stMEMSZONE[pMEMSNODEHDR->uiZoneID];
	...
	U8 szBuf[8];
	if(dOffset + TLV_SIZE > pMEMSZONE->uiMemNodeBodySize) {
		pBuf = szBuf;
		dOldLen = 0;
		dLen = TLV_SIZE;
		while(dOldLen < dLen) {
			if((dOffset + dLen - dOldLen) > pMEMSZONE->uiMemNodeBodySize) {
				dCopySize = pMEMSZONE->uiMemNodeBodySize - dOffset;
				memcpy(pBuf+dOldLen, pSetNode+dOffset, dCopySize);
				pSNode = clist_entry(nifo_ptr(pMEMSINFO, (pSNode->cont).offset_next), NIFO, cont); 
				pSetNode = (U8 *)pSNode;
				dOldLen += dCopySize;
				dOffset = (dOffset + dCopySize + NIFO_SIZE) % pMEMSZONE->uiMemNodeBodySize;
			} else {
				dCopySize = dLen - dOldLen;
				memcpy(pBuf+dOldLen, pSetNode+dOffset, dCopySize);
				dOldLen += dCopySize;
	...
	*len = pTLV->len;
	...
	if(dOffset + dLen > pMEMSZONE->uiMemNodeBodySize) {
		if((pBuf = malloc(dLen + 1)) == NULL) {
			return -errno;
		}
...
S32 nifo_copy_tlv_cont(stMEMSINFO *pMEMSINFO, U32 type, U32 len, U8 *value, U8 *node)
{
	...
	pMEMSNODEHDR = (stMEMSNODEHDR *)(pNode - stMEMSNODEHDR_SIZE);
	uiZoneID = pMEMSNODEHDR->uiZoneID;
...
S32 nifo_msg_write(stMEMSINFO *pstMEMSINFO, U32 uiMsgqID, U8 *pNode)
{
	...
	OFFSET		offset;
	U8			szBuf[BUFSIZ];
	st_MsgQ_NIFO		*pstMsgQ;
//	st_MsgQSub	*pstMsgQSub;

	pstMsgQ = (st_MsgQ_NIFO *)szBuf;
...
//	pstMsgQSub->msgid = 0;
	...
	if((dRet = nifo_node_check(pstMEMSINFO, offset)) < 0) {
		FPRINTF(LOG_BUG, "%s() @@@+++### SEND MSG : CANNOT SEND NON-ALLOCED MEMORY\n", __FUNCTION__);
//		return -1;
		exit(0);
	}
#endif

	memcpy(&szBuf[st_MsgQ_NIFO_SIZE], &offset, sizeof(OFFSET));

	dLen = st_MsgQ_NIFO_SIZE + pstMsgQ->len - sizeof(long);
...
OFFSET nifo_msg_read(stMEMSINFO *pstMEMSINFO, U32 uiMsgqID, READ_VAL_LIST *pREADVALLIST)
{
    ...
    if((dRet = msgrcv(uiMsgqID, szBuf, BUFSIZ - sizeof(long), 0, IPC_NOWAIT | MSG_NOERROR)) < 0)
    {
        ...
        {
            FPRINTF(LOG_BUG, "[FAIL:%d] MSGRCV MYQ : [%s]\n", errno, strerror(errno));
            return -errno;
        }
	...
	/* CHECK SIZE */
	pstMsgQ = (st_MsgQ_NIFO *)szBuf;
	if(dRet != pstMsgQ->len + st_MsgQ_NIFO_SIZE - sizeof(long))
	{
#ifdef	NIFO_TEST
		FPRINTF(LOG_BUG, "PROID[%d] MESSAGE SIZE ERROR RCV[%d]BODY[%d]HEAD[%d]MTYPE[%d]\n",
		pstMsgQ->procid, dRet, pstMsgQ->len, st_MsgQ_NIFO_SIZE, sizeof(long));
#endif
		return 0;
	}

	offset = (OFFSET *)&szBuf[st_MsgQ_NIFO_SIZE];

#ifdef NIFO_TEST
	if((dRet = nifo_node_check(pstMEMSINFO, *offset)) < 0) {
		FPRINTF(LOG_BUG, "@@@+++### RCV MSG : RCV NON-ALLOCED MEMORY | SEND PROCESSID[%d] OFFSET[%u]\n", pstMsgQ->procid, *offset);
//		return -1;
		exit(0);
	}
#endif
...
#ifdef JJIN
		FPRINTF(LOG_LEVEL, "%s() @@@+++### ERROR: BROKEN MEMORY !!! will be exit | ALLOC PROCESS[%s] FREE PROCESS[%s] ID[%u]ZONEID[%u]TIME[%u]ISFREE[%d]\n", __FUNCTION__,
			pMEMSNODEHDR->DebugStr, procName, pMEMSNODEHDR->uiID, pMEMSNODEHDR->uiZoneID,
			pMEMSNODEHDR->TimeSec, pMEMSNODEHDR->ucIsFree);
#else
		FPRINTF(LOG_BUG, "@@@+++### ERROR: BROKEN MEMORY !!! will be exit | ALLOC PROCESS[%s] FREE PROCESS[%s] ID[%u]ZONEID[%u]TIME[%u]ISFREE[%d]\n",
			pMEMSNODEHDR->DebugStr, pMEMSNODEHDR->DelStr, pMEMSNODEHDR->uiID, pMEMSNODEHDR->uiZoneID,
			pMEMSNODEHDR->TimeSec, pMEMSNODEHDR->ucIsFree);
#endif
//		return -1;
#if 1
...
int main(int argc, char *argv[])
{
	...
	/* 1개의 Node Alloc */
	p = nifo_node_alloc(pstMEMSINFO, pstMEMSINFO->uiMemNodeBodySize, (S8 *)__FUNCTION__);
 ...
 *     *** empty log message ***
 *
 *     Revision 1.3  2011/03/24 07:26:56  dark264sh
 *     nifo, gifo: check validatoin
 *
 *     Revision 1.2  2011/01/21 01:39:41  swpark
 *     LOG classfy
 *
 *     Revision 1.1.1.1  2011/01/11 01:33:02  jjinri
 *     DIFO
 *
 *     Revision 1.6  2011/01/05 08:57:31  swpark
 *     PATH delete
 *
 *     Revision 1.5  2010/12/29 02:53:43  swpark
 *     config file name get param
 *
 *     Revision 1.4  2010/12/28 07:23:13  swpark
 *     zone matrix add
 *
 *     Revision 1.3  2010/12/10 05:44:36  dark264sh
 *     nifo nifo_zone_conf_init시 에러 체크 추가
 *
 *     Revision 1.2  2010/12/09 08:58:28  jjinri
 *     nifo_zone_conf_init
 *
 ...
 *     ..
 *
 *     Revision 1.7  2010/11/24 05:14:52  upst_cvs
 *     JJIN
 *
 *     Revision 1.6  2010/11/24 04:30:31  upst_cvs
 *     1124
 *
 *     Revision 1.5  2010/11/10 13:56:39  upst_cvs
 *     print
 *
 *     Revision 1.4  2010/10/25 08:15:32  upst_cvs
 *     nifo_init_zone bug fix
 *
 ...
 *     WTAS2
 *
 *     Revision 1.4  2009/10/19 08:47:54  pkg
 *     nifo nifo_init_tam 추가
 *
 *     Revision 1.3  2009/08/14 14:36:47  pkg
 *     ADD DEFINE NIFO_TEST
 *
 *     Revision 1.2  2009/06/10 20:59:52  jsyoon
 *     *** empty log message ***
 *
 *     Revision 1.1  2009/06/10 16:45:50  dqms
 *     *** empty log message ***
 *
 *     Revision 1.1.1.1  2009/05/26 02:13:45  dqms
 *     Init TAF_RPPI
 *
 *     Revision 1.5  2008/11/17 11:08:31  dark264sh
 *     *** empty log message ***
 *
 *     Revision 1.4  2008/11/17 08:58:18  dark264sh
 *     64bits 작업
 *

 ...
 * Hash Library file.
 *
 *		$Id: hashg.c,v 1.3 2011/09/05 08:52:50 uamyd Exp $
 *
...
hashg_init(U32 (*hashg_func)(void*,U8*),U16 usKeyLen,  S32 (*print_key)(S8*,S8*,S32), U16 usDataLen, U32 uiHashSize)
{
	...
	memset(pstHASHGINFO->psthashnode, 0x00, uiHashSize * sizeof(char *));
...
#endif
	stHASHGNODE **pp = (stHASHGNODE **) (((OFFSET *) pstHASHGINFO->psthashnode) + 
...
#endif
	p = (stHASHGNODE *) 
		*(  ((OFFSET *) pstHASHGINFO->psthashnode) 
...
hashg_print_node(S8 *pcPrtPrefixStr,stHASHGINFO *pstHASHGINFO,stHASHGNODE *pstHASHGNODE)
{
	...
	pstHASHGINFO->print_key("KEY",(char*)pstHASHGNODE->pstKey,pstHASHGINFO->usKeyLen);
	hashg_dump_DebugString("DATA",(char*)pstHASHGNODE->pstData,pstHASHGINFO->usDataLen);
}
...
hashg_print_all(S8 *pcPrtPrefixStr,stHASHGINFO *pstHASHGINFO)
{
	...
	FPRINTF(LOG_LEVEL,"##%s : pstHASHGINFO\n",__FUNCTION__);
	hashg_print_info(pcPrtPrefixStr,pstHASHGINFO);

	FPRINTF(LOG_LEVEL,"##%s : pstHASHGNODE ...\n",__FUNCTION__);
	for(iIndex=0;iIndex < pstHASHGINFO->uiHashSize;iIndex++){
		...
		/* FPRINTF(LOG_LEVEL,"##%s : %s : [%3d] location : 0x%08x , valuep=0x%x\n",pcPrtPrefixStr,__FUNCTION__,iIndex,
		(U32) (((U32 *) pstHASHGINFO->psthashnode) + iIndex),
		*((U32 *) pstHASHGINFO->psthashnode + iIndex));*/
		for(;p;p=p->next){
			FPRINTF(LOG_LEVEL,"##%s : [%3d] p->prev=%p , p=%p , p->next=%p keylen=%d\n",__FUNCTION__,iIndex, p->prev, p, p->next,pstHASHGINFO->usKeyLen);
			pstHASHGINFO->print_key("KEY",(char*)p->pstKey,pstHASHGINFO->usKeyLen);
...
hashg_draw_all(S8 *filename,S8 *labelname,stHASHGINFO *pstHASHGINFO)
{
	...
	fp = fopen(filename,"w");
	FILEPRINT(fp,"/** @file %s\n",filename);
	FILEPRINT(fp,"pstHASHGINFO->uiHashSize = %d \\n\n",pstHASHGINFO->uiHashSize);
	FILEPRINT(fp,"pstHASHGINFO->uiLinkedCnt = %d \\n\n",pstHASHGINFO->uiLinkedCnt);
	FILEPRINT(fp,"\\dot \n	\
	digraph G{ 	\n\
	fontname=Helvetica; 	\n\
	label=\"Hash Table(%s)\"; 	\n\
	...
	for(iIndex=0;iIndex < pstHASHGINFO->uiHashSize;iIndex++){
		...
		for(;p;p=p->next){
			FILEPRINT(fp,"N%p [label = \"{ <n> %p | %3d | <p> }\"];\n", p, p,iIndex);
		}
	}
	FILEPRINT(fp,"\n\n");
	for(iIndex=0;iIndex < pstHASHGINFO->uiHashSize;iIndex++){
...
/** file hash.c
 *     $Log: hashg.c,v $
 *     Revision 1.3  2011/09/05 08:52:50  uamyd
 *     modified
 *
 *     Revision 1.2  2011/08/31 04:26:05  uamyd
 *     modified
 *
 ...
 *
 *     Revision 1.3  2011/01/11 04:09:02  uamyd
 *     modified
 *
 *     Revision 1.1.1.1  2010/08/23 01:13:07  uamyd
 *     DQMS With TOTMON, 2nd-import
 *
 *     Revision 1.2  2009/06/28 08:48:29  dark264sh
 *     *** empty log message ***
 *
 ...
 *     Init TAF_RPPI
 *
 *     Revision 1.3  2008/11/17 11:05:06  dark264sh
 *     *** empty log message ***
 *
 *     Revision 1.2  2008/11/17 08:55:18  dark264sh
 *     64bits 작업
 *
 *     Revision 1.1.1.1  2008/06/09 08:17:18  jsyoon
 *     WATAS3 PROJECT START
 *
 *     Revision 1.2  2007/08/29 07:43:41  dark264sh
 *     *** empty log message ***
 *
 *     Revision 1.1  2007/08/21 12:18:17  dark264sh
 *     no message
 *

...
THIS=${0#*/}
NTAMINST="/tamapp/BIN"
GTAMINST="/gtamapp/BIN"
WGTAMINST="/WGTAMAPP/BIN"
WNTAMINST="/WNTAMAPP/BIN"
function chksys(){
...
function usage(){
	...
	echo ""
	echo "2. Recovery"
	echo ">./apppack.sh recovery [System Type] [Block List:Default 'TOTAL']"
	echo ""
...
then
	...
	then
		...
		shift
		TEMP=${1:-"TOTAL"}
		if [ $TEMP = "TOTAL" -o $TEMP = "Total" -o $TEMP = "total" ]; then
			echo "Block List: Whole Block"
			TOTALFLAG=1
		else
			echo "Block List: "$@
